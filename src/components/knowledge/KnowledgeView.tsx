...
const KnowledgeView: React.FC<KnowledgeViewProps> = ({ knowledge }) => {
  ...
  const getThemeColors = useCallback(() => {
    ...
    return {
      ...
      bgSecondary: get('--color-bg-secondary', '#E8D5A3'),
      bgCard: get('--color-bg-card', '#FAF0D7'),
      primary: get('--color-primary', '#5D4037'),
      primaryLight: get('--color-primary-light', '#795548'),
      primaryDark: get('--color-primary-dark', '#3E2723'),
      accent: get('--color-accent', '#D4A574'),
      accentLight: get('--color-accent-light', '#E8C19A'),
      accentDark: get('--color-accent-dark', '#B8956A'),
      textPrimary: get('--color-text-primary', '#3E2723'),
      textSecondary: get('--color-text-secondary', '#5D4037'),
      textMuted: get('--color-text-muted', '#8D6E63'),
      textLight: get('--color-text-light', '#A1887F'),
      border: get('--color-border', '#C4A77D'),
      borderLight: get('--color-border-light', '#D4B896'),
      success: get('--color-success', '#558B2F'),
      warning: get('--color-warning', '#F9A825'),
      error: get('--color-error', '#C62828'),
      info: get('--color-info', '#1565C0'),
      // 对比色：在暗色主题下用白色文字，亮色主题下用深色文字
      contrastOnColor: (() => {
        const bg = get('--color-bg-primary', '#F4E4BC')
        const r = parseInt(bg.slice(1,3), 16), g = parseInt(bg.slice(3,5), 16), b = parseInt(bg.slice(5,7), 16)
        const luminance = (0.299 * r + 0.587 * g + 0.114 * b) / 255
        return luminance < 0.5 ? '#FFFFFF' : '#3E2723'
      })(),
      grayLight: get('--color-text-light', '#A1887F'),
      grayMid: get('--color-text-muted', '#8D6E63'),
      grayDark: get('--color-text-secondary', '#5D4037'),
      purple: '#9C27B0',
    }
  }, [])
  ...
  // Canvas缩放与平移状态
  const canvasTransform = useRef({ scale: 1, offsetX: 0, offsetY: 0 })
  const isDragging = useRef(false)
  const dragStart = useRef({ x: 0, y: 0, offsetX: 0, offsetY: 0 })
  const [canvasZoom, setCanvasZoom] = useState(1) // 用于UI显示
  ...
  // 判断是否有动态演示
  const hasModel = knowledge.dimensions.model.type !== 'none'

  // 获取动画步骤
  const animations = hasModel ? (knowledge.dimensions.model.animations || []) : []
  const animationSteps = animations[0]?.steps || []

  // 获取滑块配置
  const sliders = hasModel ? (knowledge.dimensions.model.config?.sliders || []) : []
  ...
  const drawSequenceLimit = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number, isCompare: boolean = false, compareType?: 'converge' | 'diverge') => {
    ...
    if (isCompare && compareType === 'diverge') {
      ...
      for (let i = 1; i <= Math.min(n, 50); i++) {
        ...
        ctx.fillStyle = i % 2 === 0 ? tc.error : tc.info
        ctx.beginPath()
      ...
      for (let i = 1; i <= n; i++) {
        ...
        ctx.fillStyle = distToLimit < epsilon ? tc.success : tc.accent
        ctx.beginPath()
      ...
      ctx.fill()
      ctx.fillStyle = tc.contrastOnColor
      ctx.beginPath()
      ctx.arc(px, py, 5, 0, Math.PI * 2)
      ctx.fill()
      ...
      ctx.fillStyle = n >= N ? tc.success : tc.error
  ...
  const drawFunctionLimit = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    // 绘制 ε 带 (水平带)
    ctx.fillStyle = withAlpha(tc.error, 0.15)
  ...
  const drawInfinitesimal = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    // 绘制三条曲线
    const colors = [tc.error, tc.info, tc.success]
    ...
    // 绘制垂直参考线
    ctx.strokeStyle = withAlpha(tc.primary, 0.5)
    ctx.lineWidth = 1
    ctx.setLineDash([4, 4])
    ctx.beginPath()
    ctx.moveTo(px, 0)
  ...
  const drawFirstOrderODE = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    // 绘制方向场 (斜率场)
    ctx.strokeStyle = withAlpha(tc.primary, 0.4)
    ctx.lineWidth = 1
    for (let i = -12; i <= 12; i++) {
      for (let j = -8; j <= 8; j++) {
        const x = i * 0.5
    ...
    const colors = [tc.info, '#7B1FA2', tc.success, tc.error, '#FF6F00']
  ...
  const drawDerivativeRules = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    const x0 = getParam('x0', 1)
    const ruleType = Math.floor(getParam('rule_type', 0))
    const infoBarHeight = 50
    const canvasHeight = height - infoBarHeight
    const centerX = width / 2
    const centerY = canvasHeight / 2
    ...
    // 定义函数对 (原函数, 导函数, 名称, 颜色)
    const rules = [
      { f: (x: number) => x * x, df: (x: number) => 2 * x, name: "乘法法则: (x²)′=2x", color: tc.error, fLabel: 'x²', dfLabel: '2x' },
      { f: (x: number) => Math.exp(x) * x * x, df: (x: number) => Math.exp(x) * (x * x + 2 * x), name: "乘法法则: (x²eˣ)′", color: tc.info, fLabel: 'x²eˣ', dfLabel: 'eˣ(x²+2x)' },
      { f: (x: number) => x / (1 + x * x), df: (x: number) => (1 - x * x) / ((1 + x * x) * (1 + x * x)), name: "除法法则: (x/(1+x²))′", color: tc.success, fLabel: 'x/(1+x²)', dfLabel: '(1-x²)/(1+x²)²' },
    ]
    const rule = rules[ruleType % rules.length]
    
    // 绘制原函数
    ctx.strokeStyle = rule.color; ctx.lineWidth = 3; ctx.beginPath()
    for (let px = 0; px < width; px++) {
      const x = (px - centerX) / scale
      const y = rule.f(x)
      const py = centerY - y * scale
      if (py > -50 && py < canvasHeight + 50) {
        if (px === 0 || centerY - rule.f((px - 1 - centerX) / scale) * scale < -50 || centerY - rule.f((px - 1 - centerX) / scale) * scale > canvasHeight + 50) ctx.moveTo(px, py)
    ...
    // 绘制导函数（虚线）
    ctx.strokeStyle = withAlpha(rule.color, 0.6); ctx.lineWidth = 2; ctx.setLineDash([6, 4]); ctx.beginPath()
    for (let px = 0; px < width; px++) {
      const x = (px - centerX) / scale
      const y = rule.df(x)
      const py = centerY - y * scale
      if (py > -50 && py < canvasHeight + 50) {
        if (px === 0 || centerY - rule.df((px - 1 - centerX) / scale) * scale < -50 || centerY - rule.df((px - 1 - centerX) / scale) * scale > canvasHeight + 50) ctx.moveTo(px, py)
        else ctx.lineTo(px, py)
      }
    }
    ...
    // 在x0处画切线
    const fVal = rule.f(x0)
    const dfVal = rule.df(x0)
    const tanLen = 2
    ctx.strokeStyle = tc.warning; ctx.lineWidth = 2
    ctx.beginPath()
    ctx.moveTo(centerX + (x0 - tanLen) * scale, centerY - (fVal - dfVal * tanLen) * scale)
    ctx.lineTo(centerX + (x0 + tanLen) * scale, centerY - (fVal + dfVal * tanLen) * scale)
    ctx.stroke()
    
    // 标注点
    const px0 = centerX + x0 * scale
    const py0 = centerY - fVal * scale
    ctx.fillStyle = tc.error; ctx.beginPath(); ctx.arc(px0, py0, 6, 0, Math.PI * 2); ctx.fill()
    ctx.fillStyle = tc.primaryDark; ctx.font = 'bold 12px "Noto Serif SC", serif'
    ctx.fillText(`x₀=${x0.toFixed(1)}`, px0 + 10, py0 - 10)
    ctx.fillText(`f'(${x0.toFixed(1)})=${dfVal.toFixed(3)}`, px0 + 10, py0 + 5)
    
    // 图例
    ctx.font = 'bold 14px "Noto Serif SC", serif'
    ctx.fillStyle = rule.color; ctx.fillRect(20, 20, 15, 15)
    ctx.fillStyle = tc.primaryDark; ctx.fillText(`f(x) = ${rule.fLabel}`, 40, 33)
    ctx.strokeStyle = withAlpha(rule.color, 0.6); ctx.setLineDash([6, 4])
    ctx.beginPath(); ctx.moveTo(20, 50); ctx.lineTo(35, 50); ctx.stroke(); ctx.setLineDash([])
    ctx.fillStyle = tc.primaryDark; ctx.fillText(`f'(x) = ${rule.dfLabel}`, 40, 55)
    ctx.strokeStyle = tc.warning; ctx.lineWidth = 2
    ctx.beginPath(); ctx.moveTo(20, 72); ctx.lineTo(35, 72); ctx.stroke()
    ctx.fillStyle = tc.primaryDark; ctx.fillText('切线', 40, 77)
    
    ctx.fillStyle = tc.primary; ctx.font = 'bold 13px serif'
    ctx.fillText(rule.name, width - 250, 30)
    
    // 底部信息栏
    ctx.fillStyle = tc.bgCard; ctx.fillRect(0, canvasHeight, width, infoBarHeight)
    ctx.strokeStyle = tc.border; ctx.lineWidth = 1; ctx.beginPath(); ctx.moveTo(0, canvasHeight); ctx.lineTo(width, canvasHeight); ctx.stroke()
    ctx.font = 'bold 13px "Noto Serif SC", serif'; ctx.fillStyle = tc.primary
    ctx.fillText(`x₀ = ${x0.toFixed(1)}`, 20, canvasHeight + 30)
    ctx.fillStyle = tc.error; ctx.fillText(`f'(x₀) = ${dfVal.toFixed(3)}`, 150, canvasHeight + 30)
    ctx.fillStyle = tc.success; ctx.fillText('切线斜率 = 导数值', 350, canvasHeight + 30)
  }, [modelState.params])
  ...
  const drawDefiniteIntegral = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    for (let i = 0; i < n; i++) {
      ...
      // 填充矩形
      ctx.fillStyle = withAlpha(tc.info, 0.4)
  ...
  const drawIndefiniteIntegral = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    const colors = [withAlpha(tc.info, 0.3), withAlpha(tc.info, 0.5), withAlpha(tc.info, 0.4), withAlpha(tc.info, 0.5), withAlpha(tc.info, 0.3)]
    
    cValues.forEach((c, idx) => {
      ctx.strokeStyle = c === C ? tc.error : colors[idx]
      ctx.lineWidth = c === C ? 4 : 2
      ctx.beginPath()
      for (let px = 0; px < width; px++) {
        const x = (px - centerX) / scale
  ...
  const drawIntegrationByParts = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    const tc = getThemeColors()
    ctx.fillStyle = tc.bgPrimary; ctx.fillRect(0, 0, width, height)
    
    const xMax = getParam('x_max', 2)
    const partType = Math.floor(getParam('part_type', 0))
    const infoBarHeight = 50
    const canvasHeight = height - infoBarHeight
    const centerX = width / 2
    const centerY = canvasHeight / 2
    ...
    // 被积函数
    const integrand = (x: number) => ex.u(x) * ex.dv(x)
    
    // 填充积分区域
    ctx.fillStyle = withAlpha(tc.info, 0.15)
    ctx.beginPath()
    ctx.moveTo(centerX, centerY)
    for (let px = 0; px <= xMax * scale; px++) {
      const x = px / scale
      const y = integrand(x)
      const canvasX = centerX + px
      const canvasY = centerY - y * scale
      if (canvasY > 0 && canvasY < canvasHeight) ctx.lineTo(canvasX, canvasY)
      else ctx.lineTo(canvasX, canvasY > canvasHeight ? 0 : canvasHeight)
    }
    ctx.lineTo(centerX + xMax * scale, centerY)
    ctx.closePath(); ctx.fill()
    
    // 绘制被积函数
    ctx.strokeStyle = tc.info; ctx.lineWidth = 3; ctx.beginPath()
    for (let px = centerX - 30; px < width; px++) {
      const x = (px - centerX) / scale
      if (partType === 2 && x <= 0) continue
      const y = integrand(x)
      const py = centerY - y * scale
      if (py > -50 && py < canvasHeight + 50) {
        if (px === Math.floor(centerX - 30) || py < -50 || py > canvasHeight + 50) ctx.moveTo(px, py)
        else ctx.lineTo(px, py)
      }
    }
    ctx.stroke()
    ...
    // 原函数（虚线）
    ctx.strokeStyle = tc.error; ctx.lineWidth = 2; ctx.setLineDash([6, 4]); ctx.beginPath()
    for (let px = centerX - 30; px < width; px++) {
      const x = (px - centerX) / scale
      if (partType === 2 && x <= 0) continue
      const y = ex.resultF(x)
      const py = centerY - y * scale
      if (py > -50 && py < canvasHeight + 50) {
        if (px === Math.floor(centerX - 30) || py < -50 || py > canvasHeight + 50) ctx.moveTo(px, py)
        else ctx.lineTo(px, py)
    ...
    // 公式标注
    ctx.fillStyle = tc.info; ctx.font = 'bold 14px "Noto Serif SC", serif'
    ctx.fillText(`∫ ${ex.uName}·${ex.dvName} dx 分部积分`, 20, 30)
    ctx.font = 'bold 12px serif'
    ctx.fillText(`设 u = ${ex.uName}, dv = ${ex.dvName}dx`, 20, 55)
    ctx.fillStyle = tc.error; ctx.font = 'bold 13px serif'
    ctx.fillText(`= ${ex.result}`, 20, 80)
    
    // 图例
    ctx.fillStyle = tc.info; ctx.fillRect(20, canvasHeight - 60, 15, 15)
    ctx.fillStyle = tc.primaryDark; ctx.fillText('被积函数', 40, canvasHeight - 48)
    ctx.strokeStyle = tc.error; ctx.setLineDash([6, 4])
    ctx.beginPath(); ctx.moveTo(20, canvasHeight - 30); ctx.lineTo(35, canvasHeight - 30); ctx.stroke(); ctx.setLineDash([])
    ctx.fillStyle = tc.primaryDark; ctx.fillText('原函数', 40, canvasHeight - 25)
    
    // 底部信息栏
    ctx.fillStyle = tc.bgCard; ctx.fillRect(0, canvasHeight, width, infoBarHeight)
    ctx.strokeStyle = tc.border; ctx.lineWidth = 1; ctx.beginPath(); ctx.moveTo(0, canvasHeight); ctx.lineTo(width, canvasHeight); ctx.stroke()
    ctx.font = 'bold 13px "Noto Serif SC", serif'; ctx.fillStyle = tc.primary
    ctx.fillText('分部积分: ∫udv = uv - ∫vdu', 20, canvasHeight + 30)
    ctx.fillStyle = tc.warning; ctx.fillText('"反对幂三指"选u优先级', 330, canvasHeight + 30)
  }, [modelState.params])
  ...
  const drawDoubleIntegral = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    // 填充底面
    ctx.fillStyle = withAlpha(tc.textMuted, 0.15)
    ctx.beginPath()
    ...
    for (const pillar of pillars) {
      ...
      const baseColor = withAlpha(tc.accent, 0.5 + intensity * 0.4)
      const sideColor = withAlpha(tc.accentDark, 0.4 + intensity * 0.4)
      const topColor = withAlpha(tc.accentLight, 0.6 + intensity * 0.3)
  ...
  const drawTripleIntegral = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    for (let i = 0; i < Math.floor(n / 2); i++) {
      ...
      // 绘制圆形切片
      ctx.fillStyle = withAlpha(tc.error, slice.alpha * 0.5)
      ctx.strokeStyle = withAlpha(tc.error, slice.alpha)
      ctx.lineWidth = 1
      ctx.beginPath()
    ...
    ctx.fillStyle = withAlpha(tc.accent, 0.4)
    ...
    for (let i = Math.floor(n / 2); i < n; i++) {
      ...
      ctx.fillStyle = withAlpha(tc.info, slice.alpha * 0.5)
      ctx.strokeStyle = withAlpha(tc.info, slice.alpha)
      ctx.lineWidth = 1
      ctx.beginPath()
      ctx.ellipse(px, py, slice.r * scale * 0.85, slice.r * scale * 0.4, 0, 0, Math.PI * 2)
      ctx.fill()
    ...
    ctx.fillStyle = withAlpha(tc.accentLight, 0.6)
    ctx.strokeStyle = tc.accentLight
    ctx.lineWidth = 2
    ctx.beginPath()
  ...
  const drawLineIntegralType1 = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    for (let i = 0; i < n; i++) {
      ...
      // 用颜色深浅表示函数值
      const intensity = y / R
      ctx.strokeStyle = withAlpha(tc.error, 0.3 + intensity * 0.7)
      ctx.lineWidth = 4
      ctx.beginPath()
      ctx.moveTo(x1, y1)
      ctx.lineTo(x2, y2)
  ...
  const drawLineIntegralType2 = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    // 绘制向量场 F = (-y, x)
    ctx.strokeStyle = withAlpha(tc.info, 0.4)
    ctx.lineWidth = 1.5
    for (let i = -4; i <= 4; i++) {
      for (let j = -4; j <= 4; j++) {
  ...
  const drawSurfaceIntegralType1 = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    const n = Math.floor(getParam('n', 10))
    const infoBarHeight = 50
    const canvasHeight = height - infoBarHeight
    const centerX = width / 2
    const centerY = canvasHeight / 2
    const R = Math.min(width, canvasHeight) * 0.25
    const scale = R / 2

    // 坐标轴
    ctx.strokeStyle = tc.primary; ctx.lineWidth = 1.5
    const axLen = R * 1.3
    const drawAxis = (x: number, y: number, z: number, label: string, color: string) => {
      const end = project3D(x * axLen / scale, y * axLen / scale, z * axLen / scale, centerX, centerY, scale)
      ctx.strokeStyle = color; ctx.beginPath(); ctx.moveTo(centerX, centerY); ctx.lineTo(end.px, end.py); ctx.stroke()
      ctx.fillStyle = color; ctx.font = 'bold 13px "Noto Serif SC", serif'; ctx.fillText(label, end.px + 5, end.py - 3)
    }
    drawAxis(1, 0, 0, 'x', tc.error)
    drawAxis(0, 1, 0, 'y', tc.success)
    drawAxis(0, 0, 1, 'z', tc.info)

    // 球面网格分割
    const nTheta = n
    const nPhi = Math.max(4, Math.floor(n / 2))

    // 绘制球面网格线
    ctx.strokeStyle = withAlpha(tc.accent, 0.5); ctx.lineWidth = 0.8

    // 纬线
    for (let j = 0; j <= nPhi; j++) {
      const phi = (j / nPhi) * Math.PI
      ctx.beginPath()
      for (let i = 0; i <= nTheta * 2; i++) {
        const theta = (i / (nTheta * 2)) * Math.PI * 2
    ...
    // 填充一个高亮面片
    const hi = Math.floor(nTheta / 4), hj = Math.floor(nPhi / 3)
    const hTheta = (hi / nTheta) * Math.PI * 2, hPhi = (hj / nPhi) * Math.PI
    const dTheta = (Math.PI * 2) / nTheta, dPhi = Math.PI / nPhi

    ctx.fillStyle = withAlpha(tc.warning, 0.5); ctx.strokeStyle = tc.warning; ctx.lineWidth = 2; ctx.beginPath()
    const corners = [
      [hTheta, hPhi], [hTheta + dTheta, hPhi], [hTheta + dTheta, hPhi + dPhi], [hTheta, hPhi + dPhi]
    ]
    corners.forEach(([t, p], idx) => {
      const x = 2 * Math.sin(p) * Math.cos(t), y = 2 * Math.sin(p) * Math.sin(t), z = 2 * Math.cos(p)
    ...
    // 标注
    ctx.fillStyle = tc.primary; ctx.font = 'bold 14px "Noto Serif SC", serif'
    ctx.fillText('球面 Σ: x²+y²+z²=R²', 20, 25)
    ctx.fillStyle = tc.warning; ctx.font = '13px "Noto Serif SC", serif'
    ctx.fillText('高亮面片: dS = R²sinφdφdθ', 20, 45)
    ctx.fillStyle = tc.info; ctx.fillText(`分割: ${nTheta}×${nPhi}`, 20, 65)

    // 面积公式
    ctx.fillStyle = tc.primary; ctx.font = 'bold 13px serif'
    ctx.fillText('∬_Σ f(x,y,z) dS = ∬_D f·√(1+z²ₓ+z²ᵧ) dxdy', width - 340, 25)
    ctx.fillStyle = tc.error; ctx.fillText('球面: dS = R²sinφ dφdθ', width - 340, 45)
    ...
    ctx.fillStyle = tc.error; ctx.fillText('M = ∬_Σ ρ dS', 250, canvasHeight + 30)
    ctx.fillStyle = tc.info; ctx.fillText(`球面积 = 4πR², n=${n}`, 420, canvasHeight + 30)
  }, [modelState.params])
  ...
  const drawSurfaceIntegralType2 = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    // 绘制球面（半透明）
    ctx.fillStyle = withAlpha(tc.borderLight, 0.15)
    ctx.strokeStyle = tc.textMuted
    ctx.lineWidth = 1
  ...
  const drawSeriesConvergence = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    for (let n = 0; n <= maxN; n++) {
      ...
      // 绘制点，当前点更大
      ctx.fillStyle = n === maxN ? tc.warning : tc.error
      ctx.beginPath()
  ...
  const drawPowerSeries = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    // 绘制历史逼近（低阶，淡色）
    const historyColors = [withAlpha(tc.error, 0.2), withAlpha(tc.info, 0.2), withAlpha(tc.info, 0.25), withAlpha(tc.success, 0.2)]
  ...
  const drawFourierSeries = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    for (let n = 1; n < currentN; n += 2) {
      ctx.strokeStyle = withAlpha(tc.error, 0.1 + (n / currentN) * 0.3)
      ctx.lineWidth = 1
      ctx.beginPath()
  ...
  const drawVectorOperations = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    const angleA = getParam('angle_a', 0.46) // 默认a=(2,1,1)的角度
    const angleB = getParam('angle_b', 1.1)
    
    ctx.fillStyle = tc.bgCard; ctx.fillRect(0, 0, width, height)
    
    const angleX = -0.5, angleY = 0.5
    ...
    // 坐标轴
    ctx.strokeStyle = tc.primary; ctx.lineWidth = 2; ctx.font = 'bold 14px "Noto Serif SC", serif'
    const xStart = project3D(-3, 0, 0), xEnd = project3D(3, 0, 0)
    ctx.beginPath(); ctx.moveTo(xStart.px, xStart.py); ctx.lineTo(xEnd.px, xEnd.py); ctx.stroke()
    ctx.fillStyle = tc.error; ctx.fillText('x', xEnd.px + 5, xEnd.py)
    const yStart = project3D(0, -3, 0), yEnd = project3D(0, 3, 0)
    ctx.beginPath(); ctx.moveTo(yStart.px, yStart.py); ctx.lineTo(yEnd.px, yEnd.py); ctx.stroke()
    ctx.fillStyle = tc.success; ctx.fillText('y', yEnd.px + 5, yEnd.py)
    const zStart = project3D(0, 0, -3), zEnd = project3D(0, 0, 3)
    ctx.beginPath(); ctx.moveTo(zStart.px, zStart.py); ctx.lineTo(zEnd.px, zEnd.py); ctx.stroke()
    ctx.fillStyle = tc.info; ctx.fillText('z', zEnd.px + 5, zEnd.py - 5)
    
    // 向量 a (由角度控制)
    const aLen = 2.5
    const ax = aLen * Math.cos(angleA), ay = aLen * Math.sin(angleA), az = 0.8
    const aEnd = project3D(ax, ay, az)
    ctx.strokeStyle = tc.error; ctx.lineWidth = 3; ctx.beginPath(); ctx.moveTo(centerX, centerY); ctx.lineTo(aEnd.px, aEnd.py); ctx.stroke()
    ctx.fillStyle = tc.error; ctx.beginPath(); ctx.arc(aEnd.px, aEnd.py, 5, 0, Math.PI * 2); ctx.fill()
    ctx.font = 'bold 13px "Noto Serif SC", serif'; ctx.fillText('a⃗', aEnd.px + 8, aEnd.py - 5)
    
    // 向量 b (由角度控制)
    const bLen = 2
    const bx = bLen * Math.cos(angleB), by = bLen * Math.sin(angleB), bz = 0.5
    const bEnd = project3D(bx, by, bz)
    ctx.strokeStyle = tc.success; ctx.lineWidth = 3; ctx.beginPath(); ctx.moveTo(centerX, centerY); ctx.lineTo(bEnd.px, bEnd.py); ctx.stroke()
    ctx.fillStyle = tc.success; ctx.beginPath(); ctx.arc(bEnd.px, bEnd.py, 5, 0, Math.PI * 2); ctx.fill()
    ctx.fillText('b⃗', bEnd.px + 8, bEnd.py - 5)
    
    // 叉积 a × b
    const cx = ay * bz - az * by, cy = az * bx - ax * bz, cz = ax * by - ay * bx
    const crossEnd = project3D(cx, cy, cz)
    ctx.strokeStyle = tc.purple; ctx.lineWidth = 3; ctx.setLineDash([5, 3]); ctx.beginPath(); ctx.moveTo(centerX, centerY); ctx.lineTo(crossEnd.px, crossEnd.py); ctx.stroke(); ctx.setLineDash([])
    ctx.fillStyle = tc.purple; ctx.beginPath(); ctx.arc(crossEnd.px, crossEnd.py, 5, 0, Math.PI * 2); ctx.fill()
    ctx.fillText('a⃗×b⃗', crossEnd.px + 8, crossEnd.py - 5)
    
    // 平行四边形
    const abEnd = project3D(ax + bx, ay + by, az + bz)
    ctx.strokeStyle = tc.warning; ctx.lineWidth = 1.5; ctx.setLineDash([3, 3])
    ctx.beginPath(); ctx.moveTo(aEnd.px, aEnd.py); ctx.lineTo(abEnd.px, abEnd.py); ctx.lineTo(bEnd.px, bEnd.py); ctx.stroke(); ctx.setLineDash([])
    
    // 计算
    const dotProduct = ax * bx + ay * by + az * bz
    const crossMag = Math.sqrt(cx * cx + cy * cy + cz * cz)
    const cosTheta = dotProduct / (Math.sqrt(ax*ax+ay*ay+az*az) * Math.sqrt(bx*bx+by*by+bz*bz))
    
    ctx.font = 'bold 13px "Noto Serif SC", serif'
    ctx.fillStyle = tc.error; ctx.fillText(`a⃗=(${ax.toFixed(1)},${ay.toFixed(1)},${az.toFixed(1)})`, 50, canvasHeight - 70)
    ctx.fillStyle = tc.success; ctx.fillText(`b⃗=(${bx.toFixed(1)},${by.toFixed(1)},${bz.toFixed(1)})`, 50, canvasHeight - 50)
    ctx.fillStyle = tc.purple; ctx.fillText(`a⃗×b⃗=(${cx.toFixed(1)},${cy.toFixed(1)},${cz.toFixed(1)})`, 50, canvasHeight - 30)
    ctx.fillStyle = tc.primary; ctx.fillText(`点积: ${dotProduct.toFixed(2)}`, 280, canvasHeight - 70)
    ctx.fillText(`夹角: ${(Math.acos(Math.max(-1, Math.min(1, cosTheta))) * 180 / Math.PI).toFixed(1)}°`, 280, canvasHeight - 50)
    ctx.fillText(`|a⃗×b⃗| = ${crossMag.toFixed(2)}`, 280, canvasHeight - 30)
    ...
    ctx.fillStyle = tc.error; ctx.fillText('点积=标量', 200, canvasHeight + 30)
    ctx.fillStyle = tc.success; ctx.fillText('叉积=向量', 330, canvasHeight + 30)
  }, [modelState.params])
  ...
  const drawPlaneAndLine = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    ctx.fillStyle = withAlpha(tc.info, 0.2)
    ctx.strokeStyle = tc.info
    ctx.lineWidth = 2
  ...
  const drawSurfaces = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    const surfaceColors = [
      withAlpha(tc.info, 0.4),
      withAlpha(tc.success, 0.4),
      withAlpha(tc.error, 0.4),
      withAlpha(tc.purple, 0.4),
      withAlpha(tc.warning, 0.4),
      withAlpha(tc.info, 0.5),
      withAlpha(tc.error, 0.6)
    ]
    const strokeColors = [
      tc.info,
      tc.success,
  ...
  const drawMultivariableBasic = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    // 填充δ邻域
    ctx.fillStyle = withAlpha(tc.warning, 0.2)
    ctx.beginPath()
    for (let angle = 0; angle <= Math.PI * 2; angle += 0.1) {
      const x = delta * Math.cos(angle)
      const y = delta * Math.sin(angle)
  ...
  const drawPartialDerivative = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    ctx.fillStyle = withAlpha(tc.warning, 0.3)
    ctx.strokeStyle = tc.warning
    ctx.lineWidth = 2
  ...
  const drawCompositeImplicit = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    const nodes = [
      { name: 'z', x: leftX, y: 70, color: tc.info },
      { name: 'u', x: leftX - 70, y: 170, color: tc.error },
      { name: 'v', x: leftX + 70, y: 170, color: tc.error },
      { name: 'x', x: leftX - 100, y: 270, color: tc.success },
      { name: 'y', x: leftX + 100, y: 270, color: tc.success },
    ]
    
    // 连线（根据t参数高亮路径）
    const highlightPhase = (t * 2) % 3
    const drawLink = (from: typeof nodes[0], to: typeof nodes[0], label: string, isActive: boolean) => {
      ctx.strokeStyle = isActive ? tc.warning : withAlpha(tc.primary, 0.4)
      ctx.lineWidth = isActive ? 3 : 1.5
      if (!isActive) ctx.setLineDash([4, 3]); else ctx.setLineDash([])
      ctx.beginPath(); ctx.moveTo(from.x, from.y + 22); ctx.lineTo(to.x, to.y - 22); ctx.stroke()
      ctx.setLineDash([])
      ctx.fillStyle = isActive ? tc.warning : tc.textMuted
      ctx.font = `${isActive ? 'bold ' : ''}11px "Noto Serif SC", serif`
      ctx.textAlign = 'center'
      ctx.fillText(label, (from.x + to.x) / 2 + (from.x < to.x ? -15 : 15), (from.y + to.y) / 2 + 5)
    }
    
    drawLink(nodes[0], nodes[1], '∂z/∂u', highlightPhase < 1)
    drawLink(nodes[0], nodes[2], '∂z/∂v', highlightPhase >= 1 && highlightPhase < 2)
    drawLink(nodes[1], nodes[3], '∂u/∂x', highlightPhase >= 2)
    drawLink(nodes[1], nodes[4], '∂u/∂y', highlightPhase < 1)
    drawLink(nodes[2], nodes[3], '∂v/∂x', highlightPhase >= 1 && highlightPhase < 2)
    drawLink(nodes[2], nodes[4], '∂v/∂y', highlightPhase >= 2)
    
    // 节点
    nodes.forEach(node => {
      ctx.fillStyle = node.color; ctx.beginPath(); ctx.arc(node.x, node.y, 22, 0, Math.PI * 2); ctx.fill()
      ctx.fillStyle = tc.contrastOnColor; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'
      ctx.font = 'bold 16px "Noto Serif SC", serif'; ctx.fillText(node.name, node.x, node.y)
    })
    ...
    ctx.font = 'bold 14px "Noto Serif SC", serif'
    ctx.fillStyle = tc.info; ctx.fillText('链式法则示例', rightX, 60)
    
    ctx.font = '13px "Noto Serif SC", serif'
    ctx.fillStyle = tc.primaryDark; ctx.fillText('设 z = eᵘ·sinv, u = x+y, v = x-y', rightX, 90)
    
    ctx.fillStyle = tc.error; ctx.fillText('∂z/∂u = eᵘsinv, ∂z/∂v = eᵘcosv', rightX, 120)
    ctx.fillStyle = tc.success; ctx.fillText('∂u/∂x = 1, ∂u/∂y = 1', rightX, 145)
    ctx.fillText('∂v/∂x = 1, ∂v/∂y = -1', rightX, 165)
    
    ctx.fillStyle = tc.warning; ctx.font = 'bold 13px "Noto Serif SC", serif'
    ctx.fillText('∂z/∂x = ∂z/∂u·∂u/∂x + ∂z/∂v·∂v/∂x', rightX, 200)
    ctx.fillText('     = eᵘsinv + eᵘcosv', rightX, 220)
    ctx.fillText('     = eˣ⁺ʸ(sin(x-y)+cos(x-y))', rightX, 240)
    
    ctx.fillStyle = tc.purple; ctx.fillText('隐函数: F(x,y,z)=0 → ∂z/∂x = -Fx/Fz', rightX, 280)
    
    // 底部信息栏
    ctx.fillStyle = tc.bgCard; ctx.fillRect(0, canvasHeight, width, infoBarHeight)
    ctx.strokeStyle = tc.border; ctx.lineWidth = 1; ctx.beginPath(); ctx.moveTo(0, canvasHeight); ctx.lineTo(width, canvasHeight); ctx.stroke()
    ctx.font = 'bold 13px "Noto Serif SC", serif'; ctx.fillStyle = tc.primary
    ctx.fillText('链式法则: 分路相乘、合路相加', 20, canvasHeight + 30)
    ctx.fillStyle = tc.error; ctx.fillText('隐函数: 由方程确定', 280, canvasHeight + 30)
  }, [modelState.params])
  ...
  const drawMultivariableExtremum = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    const surfaceType = Math.floor(getParam('surface_type', 0))
    ...
    // 坐标轴
    ctx.strokeStyle = tc.primary; ctx.lineWidth = 2; ctx.font = 'bold 12px "Noto Serif SC", serif'
    ctx.beginPath(); ctx.moveTo(project3D(-2, 0, 0).px, project3D(-2, 0, 0).py); ctx.lineTo(project3D(2.5, 0, 0).px, project3D(2.5, 0, 0).py); ctx.stroke()
    ctx.fillStyle = tc.error; ctx.fillText('x', project3D(2.6, 0, 0).px, project3D(2.6, 0, 0).py)
    ctx.beginPath(); ctx.moveTo(project3D(0, -2, 0).px, project3D(0, -2, 0).py); ctx.lineTo(project3D(0, 2.5, 0).px, project3D(0, 2.5, 0).py); ctx.stroke()
    ctx.fillStyle = tc.success; ctx.fillText('y', project3D(0, 2.7, 0).px, project3D(0, 2.7, 0).py)
    ctx.beginPath(); ctx.moveTo(project3D(0, 0, -0.5).px, project3D(0, 0, -0.5).py); ctx.lineTo(project3D(0, 0, 2.5).px, project3D(0, 0, 2.5).py); ctx.stroke()
    ctx.fillStyle = tc.info; ctx.fillText('z', project3D(0, 0, 2.7).px, project3D(0, 0, 2.7).py)
    
    // 曲面定义
    const surfaces = [
      { f: (x: number, y: number) => x * x + y * y, name: '极小值: z=x²+y²', color: tc.success, point: '极小值', zOff: 0 },
      { f: (x: number, y: number) => -(x * x + y * y), name: '极大值: z=-(x²+y²)', color: tc.error, point: '极大值', zOff: 0 },
      { f: (x: number, y: number) => x * x - y * y, name: '鞍点: z=x²-y²', color: tc.purple, point: '鞍点', zOff: 0 },
    ]
    const surf = surfaces[surfaceType % surfaces.length]
    
    // 绘制曲面
    ctx.strokeStyle = surf.color; ctx.lineWidth = 1.2
    for (let x = -1.5; x <= 1.5; x += 0.3) {
      ctx.beginPath()
      for (let y = -1.5; y <= 1.5; y += 0.1) {
        const z = surf.f(x, y) * 0.3 + surf.zOff
        const proj = project3D(x, y, z)
        if (y === -1.5) ctx.moveTo(proj.px, proj.py); else ctx.lineTo(proj.px, proj.py)
      }
      ctx.stroke()
    }
    for (let y = -1.5; y <= 1.5; y += 0.3) {
      ...
      for (let x = -1.5; x <= 1.5; x += 0.1) {
        const z = surf.f(x, y) * 0.3 + surf.zOff
        const proj = project3D(x, y, z)
        if (x === -1.5) ctx.moveTo(proj.px, proj.py); else ctx.lineTo(proj.px, proj.py)
      }
      ctx.stroke()
    }
    
    // 驻点
    const sp = project3D(0, 0, surf.zOff)
    ctx.fillStyle = tc.warning; ctx.beginPath(); ctx.arc(sp.px, sp.py, 7, 0, Math.PI * 2); ctx.fill()
    ctx.fillStyle = tc.primaryDark; ctx.font = 'bold 13px "Noto Serif SC", serif'
    ctx.fillText(surf.point + '(0,0)', sp.px + 12, sp.py - 5)
    
    // 判别信息
    ctx.font = 'bold 13px "Noto Serif SC", serif'
    const judgements = [
      { delta: 'Δ=AC-B²>0, A>0', result: '极小值点' },
      { delta: 'Δ=AC-B²>0, A<0', result: '极大值点' },
      { delta: 'Δ=AC-B²<0', result: '鞍点(非极值)' },
    ]
    const j = judgements[surfaceType % 3]
    ctx.fillStyle = surf.color; ctx.fillText(surf.name, 50, canvasHeight - 70)
    ctx.fillStyle = tc.primary; ctx.fillText(`判别: ${j.delta}`, 50, canvasHeight - 50)
    ctx.fillText(`结论: ${j.result}`, 50, canvasHeight - 30)
    ctx.fillStyle = tc.warning; ctx.fillText('驻点条件: ∇f = 0', 300, canvasHeight - 70)
    ctx.fillStyle = tc.info; ctx.fillText('A=fxx, B=fxy, C=fyy', 300, canvasHeight - 50)
    ctx.fillStyle = tc.error; ctx.fillText('Δ>0,A<0→极大; Δ>0,A>0→极小; Δ<0→鞍点', 300, canvasHeight - 30)
    
    // 底部信息栏
    ctx.fillStyle = tc.bgCard; ctx.fillRect(0, canvasHeight, width, infoBarHeight)
    ctx.strokeStyle = tc.border; ctx.lineWidth = 1; ctx.beginPath(); ctx.moveTo(0, canvasHeight); ctx.lineTo(width, canvasHeight); ctx.stroke()
    ctx.font = 'bold 13px "Noto Serif SC", serif'; ctx.fillStyle = tc.primary
    ctx.fillText('多元函数极值: 找驻点→判类型', 20, canvasHeight + 30)
    ctx.fillStyle = tc.warning; ctx.fillText(`当前: ${surf.point}`, 280, canvasHeight + 30)
  }, [modelState.params])
  ...
  const drawDeterminantDefinition = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    ctx.fillStyle = det >= 0 ? withAlpha(tc.info, 0.3) : withAlpha(tc.warning, 0.3)
    ctx.beginPath()
    ctx.moveTo(centerX, centerY)
    ctx.lineTo(centerX + a11 * scale, centerY - a21 * scale)
    ...
    ctx.fillStyle = det >= 0 ? tc.info : tc.warning
  ...
  // 矩阵的秩
  const drawMatrixRank = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    const tc = getThemeColors()
    ctx.fillStyle = tc.bgPrimary; ctx.fillRect(0, 0, width, height)

    const step = Math.floor(getParam('transform', 0))
    const infoBarHeight = 50
    const canvasHeight = height - infoBarHeight
    ...
    const descriptions = [
      '原矩阵 A = [[1,2,3],[2,4,6],[1,1,1]]',
      'R2 - 2R1 → R2: 第2行变为全零',
      'R3 - R1 → R3: 第3行变为[0,-1,-2]',
      '交换R2,R3: 阶梯形出现',
      'R1+2R2: 行最简形',
      'r(A) = 2: 非零行数=2'
    ]
    const ranks = [3, 2, 2, 2, 2, 2]

    const currentStep = Math.min(step, matrices.length - 1)
    const mat = matrices[currentStep]
    const rank = ranks[currentStep]

    // 绘制矩阵
    const cellW = 60, cellH = 40
    const matX = width / 2 - (mat[0].length * cellW) / 2 - 20
    const matY = 80

    // 矩阵括号
    ctx.strokeStyle = tc.primary; ctx.lineWidth = 3
    ctx.beginPath(); ctx.moveTo(matX, matY - 5); ctx.lineTo(matX - 10, matY - 5); ctx.lineTo(matX - 10, matY + mat.length * cellH + 5); ctx.lineTo(matX, matY + mat.length * cellH + 5); ctx.stroke()
    const rightX = matX + mat[0].length * cellW + 10
    ctx.beginPath(); ctx.moveTo(rightX, matY - 5); ctx.lineTo(rightX + 10, matY - 5); ctx.lineTo(rightX + 10, matY + mat.length * cellH + 5); ctx.lineTo(rightX, matY + mat.length * cellH + 5); ctx.stroke()

    // 矩阵元素
    ctx.font = 'bold 16px "Noto Serif SC", serif'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'
    mat.forEach((row, i) => {
      const isZeroRow = row.every(v => v === 0)
      row.forEach((val, j) => {
        const x = matX + j * cellW + cellW / 2
        const y = matY + i * cellH + cellH / 2
        if (isZeroRow) {
          ctx.fillStyle = withAlpha(tc.textMuted, 0.5)
        } else if (val === 0) {
          ctx.fillStyle = tc.textMuted
        } else if (i === 0 && val === 1 && j === 0 || (i === 1 && j === 1 && val === 1)) {
          ctx.fillStyle = tc.error // 主元
        } else {
          ctx.fillStyle = tc.primaryDark
        }
    ...
    // 秩标注
    ctx.fillStyle = tc.error; ctx.font = 'bold 18px "Noto Serif SC", serif'
    ctx.fillText(`r(A) = ${rank}`, matX - 10, matY + mat.length * cellH + 40)

    // 变换描述
    ctx.fillStyle = tc.info; ctx.font = 'bold 14px "Noto Serif SC", serif'
    ctx.fillText(`步骤 ${currentStep}: ${descriptions[currentStep]}`, 50, 45)

    // 秩的含义
    ctx.font = 'bold 13px "Noto Serif SC", serif'
    ctx.fillStyle = tc.primary
    ctx.fillText('秩 = 阶梯形非零行数 = 列空间维数 = 行空间维数', 50, canvasHeight - 70)
    ctx.fillStyle = tc.error
    ctx.fillText('初等行变换不改变矩阵的秩', 50, canvasHeight - 50)
    ctx.fillStyle = tc.success
    ctx.fillText('r(A) = r(Aᵀ), r(AB) ≤ min(r(A),r(B))', 50, canvasHeight - 30)

    // 底部信息栏
    ctx.fillStyle = tc.bgCard; ctx.fillRect(0, canvasHeight, width, infoBarHeight)
    ctx.strokeStyle = tc.border; ctx.lineWidth = 1; ctx.beginPath(); ctx.moveTo(0, canvasHeight); ctx.lineTo(width, canvasHeight); ctx.stroke()
    ctx.font = 'bold 13px "Noto Serif SC", serif'; ctx.fillStyle = tc.primary
    ctx.fillText(`变换步数: ${currentStep}/${matrices.length - 1}`, 20, canvasHeight + 30)
    ctx.fillStyle = tc.error; ctx.fillText(`r(A) = ${rank}`, 200, canvasHeight + 30)
  }, [modelState.params])

  // 矩阵方程与特殊矩阵
  const drawMatrixEquation = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    const tc = getThemeColors()
    ctx.fillStyle = tc.bgPrimary; ctx.fillRect(0, 0, width, height)

    const eqType = Math.floor(getParam('eq_type', 0))
    const infoBarHeight = 50
    const canvasHeight = height - infoBarHeight

    // 方程类型
    const equations = [
      { name: 'AX = B', desc: '若|A|≠0, 则X=A⁻¹B', example: 'A=[[1,2],[3,4]], B=[[5],[6]]', result: 'X=[[-4],[4.5]]' },
      { name: 'XA = B', desc: '若|A|≠0, 则X=BA⁻¹', example: 'X·[[1,2],[3,4]]=[[5,6]]', result: 'X=[[-1,2],[1,1]]' },
      { name: 'AXB = C', desc: '若|A|,|B|≠0, 则X=A⁻¹CB⁻¹', example: 'A,B可逆时', result: 'X=A⁻¹CB⁻¹' },
    ]
    const eq = equations[eqType % equations.length]

    // 标题
    ctx.fillStyle = tc.info; ctx.font = 'bold 16px "Noto Serif SC", serif'
    ctx.fillText(`矩阵方程: ${eq.name}`, 50, 40)
    ctx.fillStyle = tc.primary; ctx.font = '14px "Noto Serif SC", serif'
    ctx.fillText(`解法: ${eq.desc}`, 50, 65)

    // 绘制特殊矩阵类型
    const types = [
      { name: '单位阵 I', color: tc.info, rows: 3, cols: 3, val: (i: number, j: number) => i === j ? 1 : 0 },
      { name: '对角阵', color: tc.success, rows: 3, cols: 3, val: (i: number, j: number) => i === j ? (i + 1) : 0 },
      { name: '对称阵 A=Aᵀ', color: tc.error, rows: 3, cols: 3, val: (i: number, j: number) => i + j + 1 },
      { name: '上三角阵', color: tc.purple, rows: 3, cols: 3, val: (i: number, j: number) => i <= j ? (i * 3 + j + 1) : 0 },
    ]

    const cellW = 45, cellH = 32
    const startX = 50
    let startY = 100

    types.forEach((type, tIdx) => {
      const tx = startX + (tIdx % 2) * (width / 2)
      const ty = startY + Math.floor(tIdx / 2) * 160

      ctx.fillStyle = type.color; ctx.font = 'bold 13px "Noto Serif SC", serif'
      ctx.fillText(type.name, tx, ty)

      const mx = tx, my = ty + 10
      // 括号
      ctx.strokeStyle = type.color; ctx.lineWidth = 2
      ctx.beginPath(); ctx.moveTo(mx, my); ctx.lineTo(mx - 6, my); ctx.lineTo(mx - 6, my + type.rows * cellH); ctx.lineTo(mx, my + type.rows * cellH); ctx.stroke()
      const rx = mx + type.cols * cellW + 6
      ctx.beginPath(); ctx.moveTo(rx - 6, my); ctx.lineTo(rx, my); ctx.lineTo(rx, my + type.rows * cellH); ctx.lineTo(rx - 6, my + type.rows * cellH); ctx.stroke()

      ctx.font = '14px "Noto Serif SC", serif'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'
      for (let i = 0; i < type.rows; i++) {
        for (let j = 0; j < type.cols; j++) {
          const v = type.val(i, j)
          ctx.fillStyle = v === 0 ? tc.textMuted : type.color
          ctx.fillText(`${v}`, mx + j * cellW + cellW / 2, my + i * cellH + cellH / 2)
        }
      }
    })
    ctx.textAlign = 'left'; ctx.textBaseline = 'alphabetic'
    ...
    // 方程示例
    ctx.fillStyle = tc.primary; ctx.font = 'bold 13px "Noto Serif SC", serif'
    ctx.fillText(`示例: ${eq.example}`, 50, canvasHeight - 70)
    ctx.fillStyle = tc.error; ctx.fillText(`解: ${eq.result}`, 50, canvasHeight - 50)
    ctx.fillStyle = tc.info; ctx.fillText('关键: 判断A是否可逆 → 用|A|≠0或r(A)=n', 50, canvasHeight - 30)

    // 底部信息栏
    ctx.fillStyle = tc.bgCard; ctx.fillRect(0, canvasHeight, width, infoBarHeight)
    ctx.strokeStyle = tc.border; ctx.lineWidth = 1; ctx.beginPath(); ctx.moveTo(0, canvasHeight); ctx.lineTo(width, canvasHeight); ctx.stroke()
    ctx.font = 'bold 13px "Noto Serif SC", serif'; ctx.fillStyle = tc.primary
    ctx.fillText(`方程类型: ${eq.name}`, 20, canvasHeight + 30)
  ...
  const drawRandomExperiment = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    for (let i = 0; i < nSample; i++) {
      ...
      const isInA = i < aSize
      ctx.fillStyle = isInA ? tc.error : tc.primary
      ctx.beginPath()
      ctx.arc(px, py, 8, 0, Math.PI * 2)
      ctx.fill()
  ...
  const drawEventRelation = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    // A∪B 区域
    ctx.fillStyle = withAlpha(tc.info, 0.12)
    ctx.beginPath()
    ...
    // A-B 区域
    ctx.fillStyle = withAlpha(tc.error, 0.25)
    ctx.beginPath()
    ctx.arc(ax, centerY, r, 0, Math.PI * 2)
    ctx.fill()

    // B 区域覆盖（擦除A-B部分，留下交集颜色）
    ctx.fillStyle = withAlpha(tc.info, 0.25)
    ctx.beginPath()
    ctx.arc(bx, centerY, r, 0, Math.PI * 2)
    ctx.fill()

    // A∩B 区域（交集）
    ctx.fillStyle = withAlpha(tc.success, 0.35)
    ctx.save()
    ctx.beginPath()
    ctx.arc(ax, centerY, r, 0, Math.PI * 2)
    ctx.clip()
    ...
    // 补集区域
    ctx.fillStyle = withAlpha(tc.warning, 0.1)
  ...
  const drawProbabilityAxiom = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    ctx.fillStyle = withAlpha(tc.error, 0.7)
    ...
    ctx.fillStyle = withAlpha(tc.info, 0.7)
    ...
    ctx.fillStyle = withAlpha(tc.success, 0.7)
    ...
    // 绘制 P(A) 部分
    ctx.fillStyle = withAlpha(tc.error, 0.5)
    ctx.fillRect(x4, baseY - pA * barMaxH, barW, pA * barMaxH)
    // 绘制 P(B) 部分（叠加上方）
    ctx.fillStyle = withAlpha(tc.info, 0.5)
  ...
  const drawClassicalProbability = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    for (let i = 0; i < n; i++) {
      ...
      // 有利事件用红色圆形，其他用棕色
      ctx.fillStyle = isFavorable ? withAlpha(tc.error, 0.8) : withAlpha(tc.primary, 0.4)
      ctx.beginPath()
      ctx.arc(x, y, cellSize * 0.35, 0, Math.PI * 2)
      ctx.fill()

      ctx.strokeStyle = isFavorable ? tc.error : tc.primary
      ctx.lineWidth = 2
      ctx.stroke()
    ...
    ctx.fillStyle = withAlpha(tc.primary, 0.4)
    ctx.beginPath()
  ...
  const drawGeometricProbability = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    // 填充正方形区域（样本空间）
    ctx.fillStyle = withAlpha(tc.info, 0.1)
    ...
    ctx.fillStyle = withAlpha(tc.error, 0.2)
    ctx.beginPath()
    ...
    // 标签
    ctx.fillStyle = withAlpha(tc.info, 0.6)
    ctx.font = '13px "Noto Serif SC", serif'
  ...
  const drawConditionalProbability = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    // A∩B 突出显示
    ctx.fillStyle = withAlpha(tc.success, 0.4)
    ctx.save()
    ctx.beginPath()
    ctx.arc(ax, centerY, rA, 0, Math.PI * 2)
    ctx.clip()
  ...
  const drawBayesFormula = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    // 第二层分叉 A|B1
    ctx.strokeStyle = withAlpha(tc.error, 0.6)
    ctx.lineWidth = 2
    ctx.beginPath()
    ...
    // 第二层分叉 Ā|B1
    ctx.strokeStyle = withAlpha(tc.error, 0.3)
    ctx.lineWidth = 1.5
    ctx.beginPath()
    ctx.moveTo(treeMidX + 12, treeMidY1)
    ...
    // 第二层分叉 A|B2
    ctx.strokeStyle = withAlpha(tc.info, 0.6)
    ctx.lineWidth = 2
    ctx.beginPath()
    ...
    // 第二层分叉 Ā|B2
    ctx.strokeStyle = withAlpha(tc.info, 0.3)
    ctx.lineWidth = 1.5
    ctx.beginPath()
    ctx.moveTo(treeMidX + 12, treeMidY2)
  ...
  const drawIndependenceBernoulli = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    for (let i = 0; i < n; i++) {
      ...
      ctx.fillStyle = isSuccess ? tc.success : tc.error
  ...
  const drawDistributionFunction = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    // 区间阴影
    ctx.fillStyle = withAlpha(tc.error, 0.08)
  ...
  const drawBinomialDistribution = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    for (let k = 0; k <= n; k++) {
      ...
      ctx.fillStyle = isNearMean ? withAlpha(tc.error, 0.8) : withAlpha(tc.info, 0.6)
      ctx.fillRect(x, y, barW, barH)
      ctx.strokeStyle = isNearMean ? tc.error : tc.info
      ctx.lineWidth = 1.5
      ctx.strokeRect(x, y, barW, barH)
  ...
  const drawPoissonDistribution = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    for (let k = 0; k <= maxK; k++) {
      ...
      ctx.fillStyle = isNearLambda ? withAlpha(tc.success, 0.8) : withAlpha(tc.info, 0.6)
      ctx.fillRect(x, y, barW, barH)
      ctx.strokeStyle = isNearLambda ? tc.success : tc.info
      ctx.lineWidth = 1.5
      ctx.strokeRect(x, y, barW, barH)
  ...
  const drawNormalDistribution = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    // 3σ阴影区域
    const regions = [
      { start: mu - sigma, end: mu + sigma, color: withAlpha(tc.error, 0.2), label: '68.27%' },
      { start: mu - 2 * sigma, end: mu + 2 * sigma, color: withAlpha(tc.info, 0.1), label: '95.45%' },
      { start: mu - 3 * sigma, end: mu + 3 * sigma, color: withAlpha(tc.success, 0.08), label: '99.73%' },
    ]
    ...
    for (let i = 1; i <= 3; i++) {
      ...
      ctx.strokeStyle = i === 1 ? tc.error : i === 2 ? tc.info : tc.success
      ctx.lineWidth = 1
      ctx.setLineDash([3, 3])
      ctx.beginPath()
  ...
  const drawJointDistribution = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    for (let px = 0; px < plotSize; px += resolution) {
      for (let py = 0; py < plotSize; py += resolution) {
        ...
        const pr = parseInt(tc.primary.slice(1,3),16), pg = parseInt(tc.primary.slice(3,5),16), pb = parseInt(tc.primary.slice(5,7),16)
        const ir = parseInt(tc.info.slice(1,3),16), ig = parseInt(tc.info.slice(3,5),16), ib = parseInt(tc.info.slice(5,7),16)
        const r = Math.round(pr + (ir - pr) * intensity)
        const g = Math.round(pg + (ig - pg) * intensity)
        const b = Math.round(pb + (ib - pb) * intensity)
  ...
  const drawTwoDimDiscrete = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    for (let i = 0; i < m; i++) {
      ...
      for (let j = 0; j < n; j++) {
        ...
        const br = parseInt(tc.bgPrimary.slice(1,3),16), bg2 = parseInt(tc.bgPrimary.slice(3,5),16), bb = parseInt(tc.bgPrimary.slice(5,7),16)
        const ir = parseInt(tc.info.slice(1,3),16), ig = parseInt(tc.info.slice(3,5),16), ib = parseInt(tc.info.slice(5,7),16)
        const r = Math.round(br + (ir - br) * intensity)
        const g = Math.round(bg2 + (ig - bg2) * intensity)
        const b = Math.round(bb + (ib - bb) * intensity)
        ctx.fillStyle = `rgb(${r},${g},${b})`
        ctx.fillRect(cx, rowY, cellW, cellH)
        ctx.strokeStyle = tc.border
        ctx.lineWidth = 1
        ctx.strokeRect(cx, rowY, cellW, cellH)
        ctx.fillStyle = intensity > 0.5 ? tc.contrastOnColor : tc.primaryDark
        ctx.font = '12px "Noto Serif SC", serif'
        ctx.textAlign = 'center'
  ...
  const drawTwoDimContinuous = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    const colors = [tc.error, withAlpha(tc.error, 0.8), withAlpha(tc.warning, 0.7), withAlpha(tc.warning, 0.6), tc.accentLight]
  ...
  const drawConditionalDistribution = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    const contourLevels = [0.5, 1.0, 1.5, 2.0, 2.5]
    const colors = [tc.error, withAlpha(tc.error, 0.7), withAlpha(tc.warning, 0.7), withAlpha(tc.warning, 0.6), tc.accentLight]
    contourLevels.forEach((c, idx) => {
      ctx.strokeStyle = colors[idx]
      ctx.lineWidth = 1.5
      ctx.beginPath()
  ...
  const drawRVIndependence = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    const drawHeatmap = (offsetX: number, rhoVal: number, title: string) => {
      ...
      ctx.font = '12px "Noto Serif SC", serif'
      ctx.fillStyle = Math.abs(rhoVal) < 0.01 ? tc.success : tc.error
    ...
    ctx.font = '13px "Noto Serif SC", serif'
    ctx.fillText(`ρ = ${rho.toFixed(2)}`, 20, canvasHeight + 30)
    ctx.fillStyle = Math.abs(rho) < 0.01 ? tc.success : tc.error
  ...
  const drawExpectation = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    for (let k = 0; k <= n; k++) {
      ...
      const ir = parseInt(tc.info.slice(1,3),16), ig = parseInt(tc.info.slice(3,5),16), ib = parseInt(tc.info.slice(5,7),16)
      const er = parseInt(tc.error.slice(1,3),16), eg = parseInt(tc.error.slice(3,5),16), eb = parseInt(tc.error.slice(5,7),16)
      const r = Math.round(ir + (er - ir) * (1 - intensity))
      const g = Math.round(ig + (eg - ig) * (1 - intensity))
      const b = Math.round(ib + (eb - ib) * (1 - intensity))
      ctx.fillStyle = `rgb(${r},${g},${b})`
  ...
  const drawVariance = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    const sigColors = [withAlpha(tc.textLight, 0.4), withAlpha(tc.textLight, 0.6), withAlpha(tc.textMuted, 0.7), withAlpha(tc.textMuted, 0.9), tc.error]
  ...
  const drawCovarianceCorrelation = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    ctx.fillStyle = rho > 0 ? tc.success : rho < 0 ? tc.error : tc.primary
    ctx.font = 'bold 14px "Noto Serif SC", serif'
    ctx.textAlign = 'left'
  ...
  const drawMomentCovMatrix = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    ctx.fillStyle = det > 0 ? tc.success : tc.error
  ...
  const drawDistributionFeaturesSummary = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    ctx.fillStyle = withAlpha(tc.primary, 0.15)
    ...
    dists.forEach((dist, row) => {
      ...
      cells.forEach((cell, idx) => {
        ctx.fillStyle = isSelected ? tc.info : tc.primaryDark
    ...
    if (graphW > 100) {
      ...
      if (dist_type === 0) {
        ...
        for (let k = 0; k <= bn; k++) {
          const prob = binom(k)
          ctx.fillStyle = withAlpha(tc.error, 0.6)
  ...
  const drawChebyshevInequality = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    ctx.beginPath(); ctx.moveTo(width - padding, originY); ctx.lineTo(width - padding - 10, originY - 5); ctx.lineTo(width - padding - 10, originY + 5); ctx.closePath(); ctx.fillStyle = tc.primary; ctx.fill()
    ...
    for (let i = -4; i <= 4; i++) { const px = centerX + i * scaleX; ctx.fillStyle = tc.primary; ctx.font = '12px "Noto Serif SC", serif'; ctx.fillText(`${i}`, px - 5, originY + 18) }
    ctx.strokeStyle = tc.info; ctx.lineWidth = 2.5; ctx.beginPath()
    ...
    ctx.stroke()
    ctx.fillStyle = withAlpha(tc.error, 0.15); ctx.beginPath()
    ...
    ctx.strokeStyle = tc.error; ctx.lineWidth = 2; ctx.setLineDash([8, 4])
    ctx.beginPath(); ctx.moveTo(centerX - k * scaleX, originY); ctx.lineTo(centerX - k * scaleX, padding); ctx.stroke()
    ctx.beginPath(); ctx.moveTo(centerX + k * scaleX, originY); ctx.lineTo(centerX + k * scaleX, padding); ctx.stroke()
    ctx.setLineDash([])
    ctx.fillStyle = tc.error; ctx.font = 'bold 13px "Noto Serif SC", serif'
    ...
    ctx.fillStyle = tc.info; ctx.fillText(`正态实际: ${actualProb.toFixed(4)}`, originX + 280, padding - 5)
    ctx.fillStyle = tc.bgCard; ctx.fillRect(0, canvasHeight, width, infoBarHeight); ctx.strokeStyle = tc.border; ctx.lineWidth = 1; ctx.beginPath(); ctx.moveTo(0, canvasHeight); ctx.lineTo(width, canvasHeight); ctx.stroke()
    ctx.fillStyle = tc.primary; ctx.font = '13px "Noto Serif SC", serif'; ctx.fillText(`k = ${k.toFixed(1)}`, 20, canvasHeight + 30)
    ctx.fillStyle = tc.error; ctx.fillText(`切比雪夫: ≤ ${chebyshevBound.toFixed(4)}`, 120, canvasHeight + 30)
    ctx.fillStyle = tc.info; ctx.fillText(`实际值: ${actualProb.toFixed(4)}`, 300, canvasHeight + 30)
    ctx.fillStyle = tc.success; ctx.fillText(`差距: ${(chebyshevBound - actualProb).toFixed(4)}`, 460, canvasHeight + 30)
  }, [modelState.params])
  ...
  const drawLawLargeNumbers = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    ctx.beginPath(); ctx.moveTo(originX + graphWidth, originY); ctx.lineTo(originX + graphWidth - 10, originY - 5); ctx.lineTo(originX + graphWidth - 10, originY + 5); ctx.closePath(); ctx.fillStyle = tc.primary; ctx.fill()
    ...
    ctx.fillStyle = tc.primary; ctx.font = '13px "Noto Serif SC", serif'; ctx.fillText('n', originX + graphWidth - 10, originY + 25); ctx.fillText('X̄ₙ', originX - 35, padding + 5)
    for (let v = 0; v <= 1; v += 0.2) { ctx.fillStyle = tc.primary; ctx.font = '12px "Noto Serif SC", serif'; ctx.fillText(v.toFixed(1), originX - 35, originY - v * scaleY + 4) }
    const pLineY = originY - p * scaleY
    ctx.strokeStyle = tc.error; ctx.lineWidth = 2; ctx.setLineDash([8, 4]); ctx.beginPath(); ctx.moveTo(originX, pLineY); ctx.lineTo(originX + graphWidth, pLineY); ctx.stroke(); ctx.setLineDash([])
    ctx.fillStyle = tc.error; ctx.font = 'bold 13px "Noto Serif SC", serif'; ctx.fillText(`p = ${p.toFixed(2)}`, originX + graphWidth - 70, pLineY - 10)
    ...
    ctx.strokeStyle = tc.info; ctx.lineWidth = 2; ctx.beginPath()
    ...
    ctx.fillStyle = tc.error; ctx.beginPath(); ctx.arc(lastPx, lastPy, 6, 0, Math.PI * 2); ctx.fill()
    ctx.fillStyle = tc.primaryDark; ctx.font = 'bold 13px "Noto Serif SC", serif'; ctx.fillText(`(${n}, ${lastFreq.toFixed(4)})`, lastPx - 80, lastPy - 12)
    ctx.fillStyle = withAlpha(tc.info, 0.1); ctx.fillRect(originX, pLineY - 0.05 * scaleY, graphWidth, 0.1 * scaleY)
    ctx.fillStyle = tc.primaryDark; ctx.font = 'bold 13px "Noto Serif SC", serif'; ctx.fillText(`大数定律: X̄ₙ → p (n→∞)`, originX, padding - 10)
    ctx.fillStyle = tc.bgCard; ctx.fillRect(0, canvasHeight, width, infoBarHeight); ctx.strokeStyle = tc.border; ctx.lineWidth = 1; ctx.beginPath(); ctx.moveTo(0, canvasHeight); ctx.lineTo(width, canvasHeight); ctx.stroke()
    ctx.fillStyle = tc.primary; ctx.font = '13px "Noto Serif SC", serif'; ctx.fillText(`n = ${n}`, 20, canvasHeight + 30)
    ctx.fillStyle = tc.error; ctx.fillText(`p = ${p.toFixed(2)}`, 120, canvasHeight + 30); ctx.fillStyle = tc.info; ctx.fillText(`频率 = ${lastFreq.toFixed(4)}`, 230, canvasHeight + 30)
    ctx.fillStyle = tc.success; ctx.fillText(`偏差 = ${Math.abs(lastFreq - p).toFixed(4)}`, 420, canvasHeight + 30)
  }, [modelState.params])
  ...
  const drawCentralLimitTheorem = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    ctx.fillStyle = tc.primary; ctx.font = '13px "Noto Serif SC", serif'; ctx.fillText('z', originX + graphWidth + 5, originY + 5)
    for (let v = -4; v <= 4; v++) { ctx.fillStyle = tc.primary; ctx.font = '12px "Noto Serif SC", serif'; ctx.fillText(`${v}`, originX + (v - xMin) * scaleX - 5, originY + 18) }
    ctx.strokeStyle = tc.error; ctx.lineWidth = 2; ctx.beginPath()
    ...
    ctx.fillStyle = withAlpha(tc.info, 0.35); ctx.strokeStyle = tc.info; ctx.lineWidth = 1
    ...
    ctx.fillStyle = tc.primaryDark; ctx.font = 'bold 13px "Noto Serif SC", serif'; ctx.fillText(`中心极限定理: (ΣXᵢ-nμ)/√(nσ²) → N(0,1)`, originX, padding - 20)
    ctx.fillStyle = tc.info; ctx.fillText(`原始分布: ${distNames[distType]}`, originX, padding - 5); ctx.fillStyle = tc.error; ctx.fillText(`N(0,1) 标准正态`, originX + 250, padding - 5)
    ctx.fillStyle = tc.bgCard; ctx.fillRect(0, canvasHeight, width, infoBarHeight); ctx.strokeStyle = tc.border; ctx.lineWidth = 1; ctx.beginPath(); ctx.moveTo(0, canvasHeight); ctx.lineTo(width, canvasHeight); ctx.stroke()
    ctx.fillStyle = tc.primary; ctx.font = '13px "Noto Serif SC", serif'; ctx.fillText(`n = ${n}`, 20, canvasHeight + 30); ctx.fillStyle = tc.info; ctx.fillText(`原始分布: ${distNames[distType]}`, 100, canvasHeight + 30); ctx.fillStyle = tc.error; ctx.fillText('红色: N(0,1)', 330, canvasHeight + 30)
  }, [modelState.params])
  ...
  const drawPopulationSample = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    for (let i = 0; i <= 10; i++) { const x = originX + (graphWidth / 10) * i; ctx.beginPath(); ctx.moveTo(x, padding); ctx.lineTo(x, canvasHeight - padding); ctx.stroke() }
    for (let i = 0; i <= 10; i++) { const y = originY - (graphHeight / 10) * i; ctx.beginPath(); ctx.moveTo(padding, y); ctx.lineTo(width - padding, y); ctx.stroke() }
    ctx.strokeStyle = tc.primary; ctx.lineWidth = 2; ctx.beginPath(); ctx.moveTo(originX, originY); ctx.lineTo(originX + graphWidth, originY); ctx.stroke(); ctx.beginPath(); ctx.moveTo(originX, originY); ctx.lineTo(originX, padding); ctx.stroke()
    ctx.beginPath(); ctx.moveTo(originX + graphWidth, originY); ctx.lineTo(originX + graphWidth - 10, originY - 5); ctx.lineTo(originX + graphWidth - 10, originY + 5); ctx.closePath(); ctx.fillStyle = tc.primary; ctx.fill()
    ctx.beginPath(); ctx.moveTo(originX, padding); ctx.lineTo(originX - 5, padding + 10); ctx.lineTo(originX + 5, padding + 10); ctx.closePath(); ctx.fill()
    ctx.fillStyle = tc.primary; ctx.font = '13px "Noto Serif SC", serif'; ctx.fillText('x', originX + graphWidth + 5, originY + 5); ctx.fillText('f(x)', originX - 35, padding + 5)
    for (let v = Math.ceil(xMin); v <= Math.floor(xMax); v++) { ctx.fillStyle = tc.primary; ctx.font = '12px "Noto Serif SC", serif'; ctx.fillText(`${v}`, originX + (v - xMin) * scaleX - 5, originY + 18) }
    ctx.strokeStyle = tc.info; ctx.lineWidth = 2.5; ctx.beginPath()
    for (let px = 0; px <= graphWidth; px++) { const x = xMin + px / scaleX; const y = normalPDF(x); if (px === 0) ctx.moveTo(originX + px, originY - y * scaleY); else ctx.lineTo(originX + px, originY - y * scaleY) }
    ctx.stroke()
    let rng = 42; const rand = () => { rng = (rng * 1103515245 + 12345) & 0x7fffffff; return rng / 0x7fffffff }
    ...
    let sampleSum = 0
    for (let i = 0; i < sampleSize; i++) { const x = mu + sigma * boxMuller(); sampleSum += x; const px = originX + (x - xMin) * scaleX; ctx.fillStyle = tc.error; ctx.beginPath(); ctx.arc(px, originY - 5 + (rand() - 0.5) * 15, 4, 0, Math.PI * 2); ctx.fill() }
    const sampleMean = sampleSum / sampleSize; const meanPx = originX + (sampleMean - xMin) * scaleX
    ctx.strokeStyle = tc.success; ctx.lineWidth = 2; ctx.setLineDash([6, 3]); ctx.beginPath(); ctx.moveTo(meanPx, originY); ctx.lineTo(meanPx, padding); ctx.stroke(); ctx.setLineDash([])
    ctx.fillStyle = tc.success; ctx.font = 'bold 13px "Noto Serif SC", serif'; ctx.fillText(`X̄ = ${sampleMean.toFixed(3)}`, meanPx + 5, padding + 20)
    ctx.fillStyle = tc.primaryDark; ctx.font = 'bold 13px "Noto Serif SC", serif'; ctx.fillText(`总体 N(${mu.toFixed(1)}, ${sigma.toFixed(1)}²) 与样本`, originX, padding - 10)
    ctx.fillStyle = tc.bgCard; ctx.fillRect(0, canvasHeight, width, infoBarHeight); ctx.strokeStyle = tc.border; ctx.lineWidth = 1; ctx.beginPath(); ctx.moveTo(0, canvasHeight); ctx.lineTo(width, canvasHeight); ctx.stroke()
    ctx.fillStyle = tc.primary; ctx.font = '13px "Noto Serif SC", serif'; ctx.fillText(`μ = ${mu.toFixed(1)}`, 20, canvasHeight + 30); ctx.fillStyle = tc.error; ctx.fillText(`σ = ${sigma.toFixed(1)}`, 120, canvasHeight + 30)
    ctx.fillStyle = tc.success; ctx.fillText(`样本量 = ${sampleSize}`, 230, canvasHeight + 30); ctx.fillText(`X̄ = ${sampleMean.toFixed(3)}`, 370, canvasHeight + 30)
  }, [modelState.params])
  ...
  const drawSamplingDistributions = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    ctx.beginPath(); ctx.moveTo(originX, padding); ctx.lineTo(originX - 5, padding + 10); ctx.lineTo(originX + 5, padding + 10); ctx.closePath(); ctx.fill()
    ctx.fillStyle = tc.primary; ctx.font = '13px "Noto Serif SC", serif'; ctx.fillText('x', originX + graphWidth + 5, originY + 5); ctx.fillText('f(x)', originX - 35, padding + 5)
    ctx.strokeStyle = tc.info; ctx.lineWidth = 2.5; ctx.beginPath(); let started = false
    ...
    ctx.stroke()
    const eX = originX + n * scaleX; ctx.strokeStyle = tc.error; ctx.lineWidth = 2; ctx.setLineDash([8, 4]); ctx.beginPath(); ctx.moveTo(eX, originY); ctx.lineTo(eX, originY - chi2PDF(n, n) * scaleY - 10); ctx.stroke(); ctx.setLineDash([])
    ctx.fillStyle = tc.error; ctx.font = 'bold 13px "Noto Serif SC", serif'; ctx.fillText(`E = ${n}`, eX - 15, originY + 30); ctx.fillText(`D = ${2 * n}`, eX + 25, originY + 30)
    ctx.fillStyle = tc.primaryDark; ctx.font = 'bold 13px "Noto Serif SC", serif'; ctx.fillText(`χ²(${n}) 分布`, originX, padding - 10)
    ctx.fillStyle = tc.bgCard; ctx.fillRect(0, canvasHeight, width, infoBarHeight); ctx.strokeStyle = tc.border; ctx.lineWidth = 1; ctx.beginPath(); ctx.moveTo(0, canvasHeight); ctx.lineTo(width, canvasHeight); ctx.stroke()
    ctx.fillStyle = tc.primary; ctx.font = '13px "Noto Serif SC", serif'; ctx.fillText(`自由度 n = ${n}`, 20, canvasHeight + 30); ctx.fillStyle = tc.error; ctx.fillText(`E(X) = ${n}`, 170, canvasHeight + 30); ctx.fillText(`D(X) = ${2 * n}`, 280, canvasHeight + 30)
  }, [modelState.params])
  ...
  const drawNormalSamplingTheorem = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    const drawSubPlot = (startX: number, title: string, drawContent: (c: CanvasRenderingContext2D, ox: number, oy: number, gw: number, gh: number) => void) => {
      ...
      ctx.strokeStyle = tc.primary; ctx.lineWidth = 1.5; ctx.beginPath(); ctx.moveTo(ox, oy); ctx.lineTo(ox + gw, oy); ctx.stroke(); ctx.beginPath(); ctx.moveTo(ox + gw / 2, oy); ctx.lineTo(ox + gw / 2, oy - gh); ctx.stroke()
      ctx.fillStyle = tc.primaryDark; ctx.font = 'bold 12px "Noto Serif SC", serif'; ctx.fillText(title, ox + 5, oy - gh - 5); drawContent(ctx, ox, oy, gw, gh)
    }
    drawSubPlot(padding, `X̄ ~ N(μ, σ²/n)`, (c, ox, oy, gw, gh) => {
      ...
      c.strokeStyle = tc.info; c.lineWidth = 2; c.beginPath()
      ...
      c.stroke(); c.fillStyle = tc.info; c.font = '11px "Noto Serif SC", serif'; c.fillText(`σ/√n = ${sigXbar.toFixed(3)}`, ox + 5, oy - 15)
    })
    drawSubPlot(padding + subWidth + 20, `(n-1)S²/σ² ~ χ²(${n - 1})`, (c, ox, oy, gw, gh) => {
      ...
      c.strokeStyle = tc.success; c.lineWidth = 2; c.beginPath(); let st = false
      ...
      c.stroke(); c.fillStyle = tc.success; c.font = '11px "Noto Serif SC", serif'; c.fillText(`df = ${df}`, ox + 5, oy - 15)
    })
    drawSubPlot(padding + (subWidth + 20) * 2, `X̄ 与 S² 独立`, (c, ox, oy, gw, gh) => {
      ...
      for (let i = 0; i < 200; i++) { const xbar = mu + sigXbar * bm(); let chi2Val = 0; for (let j = 0; j < n - 1; j++) { const z = bm(); chi2Val += z * z } const s2 = chi2Val * sigma * sigma / (n - 1); const px = centerX + (xbar - mu) / (3 * sigXbar) * (gw / 2); const py = centerY - (s2 - sigma * sigma) / (3 * sigma * sigma) * (gh / 2); if (px > ox && px < ox + gw && py > oy - gh && py < oy) { c.fillStyle = withAlpha(tc.purple, 0.4); c.beginPath(); c.arc(px, py, 2.5, 0, Math.PI * 2); c.fill() } }
      c.fillStyle = tc.purple; c.font = '11px "Noto Serif SC", serif'; c.fillText('散点无相关性', ox + 5, oy - 15); c.fillText('→ 相互独立', ox + 5, oy - 3)
    })
    ctx.fillStyle = tc.bgCard; ctx.fillRect(0, canvasHeight, width, infoBarHeight); ctx.strokeStyle = tc.border; ctx.lineWidth = 1; ctx.beginPath(); ctx.moveTo(0, canvasHeight); ctx.lineTo(width, canvasHeight); ctx.stroke()
    ctx.fillStyle = tc.primary; ctx.font = '13px "Noto Serif SC", serif'; ctx.fillText(`n = ${n}, σ = ${sigma.toFixed(1)}`, 20, canvasHeight + 30)
    ctx.fillStyle = tc.info; ctx.fillText(`X̄~N(μ,σ²/n)`, 180, canvasHeight + 30); ctx.fillStyle = tc.success; ctx.fillText(`(n-1)S²/σ²~χ²(${n - 1})`, 320, canvasHeight + 30); ctx.fillStyle = tc.purple; ctx.fillText('X̄⊥S²', 500, canvasHeight + 30)
  }, [modelState.params])
  ...
  const drawOrderStatistics = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    ctx.strokeStyle = withAlpha(tc.border, 0.3); ctx.lineWidth = 1
    for (let i = 0; i <= 10; i++) { const x = originX + (graphWidth / 10) * i; ctx.beginPath(); ctx.moveTo(x, padding); ctx.lineTo(x, canvasHeight - padding); ctx.stroke() }
    const axisY = canvasHeight / 2; ctx.strokeStyle = tc.primary; ctx.lineWidth = 2; ctx.beginPath(); ctx.moveTo(originX, axisY); ctx.lineTo(originX + graphWidth, axisY); ctx.stroke()
    ctx.beginPath(); ctx.moveTo(originX + graphWidth, axisY); ctx.lineTo(originX + graphWidth - 10, axisY - 5); ctx.lineTo(originX + graphWidth - 10, axisY + 5); ctx.closePath(); ctx.fillStyle = tc.primary; ctx.fill()
    for (let v = 0; v <= 1; v += 0.1) { const px = originX + (v - xMin) * scaleX; ctx.strokeStyle = tc.primary; ctx.lineWidth = 1; ctx.beginPath(); ctx.moveTo(px, axisY - 5); ctx.lineTo(px, axisY + 5); ctx.stroke(); if (v === 0 || v === 0.5 || v === 1) { ctx.fillStyle = tc.primary; ctx.font = '12px "Noto Serif SC", serif'; ctx.fillText(v.toFixed(1), px - 10, axisY + 25) } }
    for (let i = 0; i < n; i++) { const px = originX + (samples[i] - xMin) * scaleX; const isKth = (i === k - 1); ctx.fillStyle = isKth ? tc.error : tc.info; ctx.beginPath(); ctx.arc(px, axisY, isKth ? 8 : 5, 0, Math.PI * 2); ctx.fill(); ctx.fillStyle = isKth ? tc.error : tc.primary; ctx.font = isKth ? 'bold 12px "Noto Serif SC", serif' : '11px "Noto Serif SC", serif'; ctx.fillText(`X₍${i + 1}₎`, px - 10, axisY - (isKth ? 20 : 15)); ctx.fillText(`${samples[i].toFixed(3)}`, px - 15, axisY + (isKth ? 35 : 30)) }
    const kthPx = originX + (samples[k - 1] - xMin) * scaleX; ctx.strokeStyle = tc.error; ctx.lineWidth = 2; ctx.setLineDash([6, 3]); ctx.beginPath(); ctx.moveTo(kthPx, axisY - 40); ctx.lineTo(kthPx, axisY + 40); ctx.stroke(); ctx.setLineDash([])
    ctx.fillStyle = tc.primaryDark; ctx.font = 'bold 13px "Noto Serif SC", serif'; ctx.fillText(`顺序统计量: X₍₁₎ ≤ X₍₂₎ ≤ ... ≤ X₍${n}₎`, originX, padding)
    ctx.fillStyle = tc.error; ctx.fillText(`第${k}个顺序统计量 X₍${k}₎ = ${samples[k - 1].toFixed(3)}`, originX, padding + 20)
    ctx.fillStyle = tc.bgCard; ctx.fillRect(0, canvasHeight, width, infoBarHeight); ctx.strokeStyle = tc.border; ctx.lineWidth = 1; ctx.beginPath(); ctx.moveTo(0, canvasHeight); ctx.lineTo(width, canvasHeight); ctx.stroke()
    ctx.fillStyle = tc.primary; ctx.font = '13px "Noto Serif SC", serif'; ctx.fillText(`n = ${n}`, 20, canvasHeight + 30); ctx.fillStyle = tc.error; ctx.fillText(`k = ${k}`, 100, canvasHeight + 30); ctx.fillText(`X₍${k}₎ = ${samples[k - 1].toFixed(3)}`, 180, canvasHeight + 30)
    ctx.fillStyle = tc.success; ctx.fillText(`E[X₍${k}₎] = k/(n+1) = ${(k / (n + 1)).toFixed(3)}`, 350, canvasHeight + 30)
  }, [modelState.params])
  ...
  const drawMomentEstimation = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    const tc = getThemeColors()
    const n = Math.floor(getParam('n', 20)); ctx.fillStyle = tc.bgPrimary; ctx.fillRect(0, 0, width, height)
    ...
    ctx.beginPath(); ctx.moveTo(originX, padding); ctx.lineTo(originX - 5, padding + 10); ctx.lineTo(originX + 5, padding + 10); ctx.closePath(); ctx.fill()
    ctx.fillStyle = tc.primary; ctx.font = '13px "Noto Serif SC", serif'; ctx.fillText('x', originX + graphWidth + 5, originY + 5); ctx.fillText('f(x)', originX - 35, padding + 5)
    for (let v = 0; v <= 3; v++) { ctx.fillStyle = tc.primary; ctx.font = '12px "Noto Serif SC", serif'; ctx.fillText(`${v}`, originX + (v - xMin) * scaleX - 5, originY + 18) }
    ctx.strokeStyle = tc.info; ctx.lineWidth = 2.5
    ...
    let rng = 42; const rand = () => { rng = (rng * 1103515245 + 12345) & 0x7fffffff; return rng / 0x7fffffff }
    let sampleSum = 0
    for (let i = 0; i < n; i++) { const x = theta * rand(); sampleSum += x; const px = originX + (x - xMin) * scaleX; ctx.fillStyle = tc.error; ctx.beginPath(); ctx.arc(px, originY - 5 + (rand() - 0.5) * 10, 3, 0, Math.PI * 2); ctx.fill() }
    const sampleMean = sampleSum / n; const thetaHat = 2 * sampleMean
    const meanPx = originX + (sampleMean - xMin) * scaleX; ctx.strokeStyle = tc.success; ctx.lineWidth = 2; ctx.setLineDash([6, 3]); ctx.beginPath(); ctx.moveTo(meanPx, originY); ctx.lineTo(meanPx, originY - maxPDF * scaleY - 20); ctx.stroke(); ctx.setLineDash([])
    ctx.fillStyle = tc.success; ctx.font = 'bold 12px "Noto Serif SC", serif'; ctx.fillText(`X̄=${sampleMean.toFixed(3)}`, meanPx + 5, originY - maxPDF * scaleY - 25)
    const thetaHatPx = originX + (thetaHat - xMin) * scaleX; ctx.strokeStyle = tc.purple; ctx.lineWidth = 2; ctx.setLineDash([6, 3]); ctx.beginPath(); ctx.moveTo(thetaHatPx, originY); ctx.lineTo(thetaHatPx, originY - maxPDF * scaleY - 40); ctx.stroke(); ctx.setLineDash([])
    ctx.fillStyle = tc.purple; ctx.font = 'bold 13px "Noto Serif SC", serif'; ctx.fillText(`θ̂=2X̄=${thetaHat.toFixed(3)}`, thetaHatPx + 5, originY - maxPDF * scaleY - 45)
    ctx.fillStyle = tc.info; ctx.fillText(`θ=${theta}`, originX + (theta - xMin) * scaleX - 30, originY - maxPDF * scaleY + 20)
    ctx.fillStyle = tc.primaryDark; ctx.font = 'bold 13px "Noto Serif SC", serif'; ctx.fillText(`矩估计: E[X]=θ/2 → 用X̄替换E[X] → θ̂=2X̄`, originX, padding - 5)
    ctx.fillStyle = tc.bgCard; ctx.fillRect(0, canvasHeight, width, infoBarHeight); ctx.strokeStyle = tc.border; ctx.lineWidth = 1; ctx.beginPath(); ctx.moveTo(0, canvasHeight); ctx.lineTo(width, canvasHeight); ctx.stroke()
    ctx.fillStyle = tc.primary; ctx.font = '13px "Noto Serif SC", serif'; ctx.fillText(`n = ${n}`, 20, canvasHeight + 30); ctx.fillStyle = tc.success; ctx.fillText(`X̄ = ${sampleMean.toFixed(3)}`, 100, canvasHeight + 30)
    ctx.fillStyle = tc.purple; ctx.fillText(`θ̂ = ${thetaHat.toFixed(3)}`, 240, canvasHeight + 30); ctx.fillStyle = tc.info; ctx.fillText(`θ = ${theta}`, 380, canvasHeight + 30)
  }, [modelState.params])
  ...
  const drawMaximumLikelihood = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    ctx.beginPath(); ctx.moveTo(originX + graphWidth, originY); ctx.lineTo(originX + graphWidth - 10, originY - 5); ctx.lineTo(originX + graphWidth - 10, originY + 5); ctx.closePath(); ctx.fillStyle = tc.primary; ctx.fill()
    ctx.beginPath(); ctx.moveTo(originX, padding); ctx.lineTo(originX - 5, padding + 10); ctx.lineTo(originX + 5, padding + 10); ctx.closePath(); ctx.fill()
    ctx.fillStyle = tc.primary; ctx.font = '13px "Noto Serif SC", serif'; ctx.fillText('θ', originX + graphWidth + 5, originY + 5); ctx.fillText('L(θ)', originX - 35, padding + 5)
    for (let v = 0; v <= 1; v += 0.2) { ctx.fillStyle = tc.primary; ctx.font = '12px "Noto Serif SC", serif'; ctx.fillText(v.toFixed(1), originX + v * scaleX - 10, originY + 18) }
    ctx.strokeStyle = tc.info; ctx.lineWidth = 2.5; ctx.beginPath()
    ...
    ctx.fillStyle = tc.error; ctx.beginPath(); ctx.arc(mlePx, mlePy, 6, 0, Math.PI * 2); ctx.fill()
    ctx.fillStyle = tc.error; ctx.font = 'bold 13px "Noto Serif SC", serif'; ctx.fillText(`MLE: θ̂ = X̄ = ${mle.toFixed(3)}`, mlePx + 10, mlePy - 10)
    ctx.strokeStyle = tc.error; ctx.lineWidth = 2; ctx.setLineDash([6, 3]); ctx.beginPath(); ctx.moveTo(mlePx, originY); ctx.lineTo(mlePx, mlePy); ctx.stroke(); ctx.setLineDash([])
    const trueThetaPx = originX + theta * scaleX; ctx.strokeStyle = tc.success; ctx.lineWidth = 2; ctx.setLineDash([4, 4]); ctx.beginPath(); ctx.moveTo(trueThetaPx, originY); ctx.lineTo(trueThetaPx, originY - likelihood(theta) * scaleY); ctx.stroke(); ctx.setLineDash([])
    ctx.fillStyle = tc.success; ctx.font = '12px "Noto Serif SC", serif'; ctx.fillText(`θ=${theta.toFixed(2)}`, trueThetaPx + 5, originY + 18)
    ctx.fillStyle = tc.primaryDark; ctx.font = 'bold 13px "Noto Serif SC", serif'; ctx.fillText(`极大似然估计: L(θ) = θ^${sumX}(1-θ)^${n - sumX}`, originX, padding - 10)
    ctx.fillStyle = tc.bgCard; ctx.fillRect(0, canvasHeight, width, infoBarHeight); ctx.strokeStyle = tc.border; ctx.lineWidth = 1; ctx.beginPath(); ctx.moveTo(0, canvasHeight); ctx.lineTo(width, canvasHeight); ctx.stroke()
    ctx.fillStyle = tc.primary; ctx.font = '13px "Noto Serif SC", serif'; ctx.fillText(`n = ${n}`, 20, canvasHeight + 30); ctx.fillStyle = tc.error; ctx.fillText(`Σxᵢ = ${sumX}`, 100, canvasHeight + 30); ctx.fillText(`θ̂ = ${mle.toFixed(3)}`, 220, canvasHeight + 30)
    ctx.fillStyle = tc.success; ctx.fillText(`θ = ${theta.toFixed(2)}`, 350, canvasHeight + 30)
  }, [modelState.params])
  ...
  const drawEstimatorCriteria = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    for (let i = 0; i <= 10; i++) { const x = originX + (graphWidth / 10) * i; ctx.beginPath(); ctx.moveTo(x, padding); ctx.lineTo(x, canvasHeight - padding); ctx.stroke() }
    for (let i = 0; i <= 10; i++) { const y = originY - (graphHeight / 10) * i; ctx.beginPath(); ctx.moveTo(padding, y); ctx.lineTo(width - padding, y); ctx.stroke() }
    ctx.strokeStyle = tc.primary; ctx.lineWidth = 2; ctx.beginPath(); ctx.moveTo(originX, originY); ctx.lineTo(originX + graphWidth, originY); ctx.stroke(); const yAxisX = originX + graphWidth / 2; ctx.beginPath(); ctx.moveTo(yAxisX, originY); ctx.lineTo(yAxisX, padding); ctx.stroke()
    ctx.beginPath(); ctx.moveTo(originX + graphWidth, originY); ctx.lineTo(originX + graphWidth - 10, originY - 5); ctx.lineTo(originX + graphWidth - 10, originY + 5); ctx.closePath(); ctx.fillStyle = tc.primary; ctx.fill()
    ctx.beginPath(); ctx.moveTo(yAxisX, padding); ctx.lineTo(yAxisX - 5, padding + 10); ctx.lineTo(yAxisX + 5, padding + 10); ctx.closePath(); ctx.fill()
    ctx.fillStyle = tc.primary; ctx.font = '13px "Noto Serif SC", serif'; ctx.fillText('θ̂', originX + graphWidth + 5, originY + 5)
    ctx.strokeStyle = tc.error; ctx.lineWidth = 2.5; ctx.beginPath()
    ...
    ctx.stroke()
    ctx.strokeStyle = tc.success; ctx.lineWidth = 2.5; ctx.beginPath()
    ...
    ctx.stroke()
    ctx.strokeStyle = tc.primary; ctx.lineWidth = 1.5; ctx.setLineDash([6, 3]); ctx.beginPath(); ctx.moveTo(yAxisX, originY); ctx.lineTo(yAxisX, padding); ctx.stroke(); ctx.setLineDash([])
    ctx.fillStyle = tc.error; ctx.font = 'bold 12px "Noto Serif SC", serif'; ctx.fillText(`θ̂₁: σ = ${sigma1.toFixed(3)}`, originX, padding - 30)
    ctx.fillStyle = tc.success; ctx.fillText(`θ̂₂: σ = ${sigma2.toFixed(3)} (更有效)`, originX, padding - 12)
    ctx.fillStyle = tc.primaryDark; ctx.font = 'bold 13px "Noto Serif SC", serif'; ctx.fillText('无偏性: E[θ̂] = θ', originX + 250, padding - 30); ctx.fillText('有效性: D[θ̂₂] < D[θ̂₁] → θ̂₂更有效', originX + 250, padding - 12)
    ctx.fillStyle = tc.bgCard; ctx.fillRect(0, canvasHeight, width, infoBarHeight); ctx.strokeStyle = tc.border; ctx.lineWidth = 1; ctx.beginPath(); ctx.moveTo(0, canvasHeight); ctx.lineTo(width, canvasHeight); ctx.stroke()
    ctx.fillStyle = tc.primary; ctx.font = '13px "Noto Serif SC", serif'; ctx.fillText(`n = ${n}`, 20, canvasHeight + 30); ctx.fillStyle = tc.error; ctx.fillText(`D[θ̂₁] = ${(sigma1 * sigma1).toFixed(4)}`, 100, canvasHeight + 30)
    ctx.fillStyle = tc.success; ctx.fillText(`D[θ̂₂] = ${(sigma2 * sigma2).toFixed(4)}`, 280, canvasHeight + 30); ctx.fillStyle = tc.success; ctx.fillText('θ̂₂更有效', 440, canvasHeight + 30)
  }, [modelState.params])
  ...
  const drawIntervalEstimation = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    for (let i = 0; i <= 10; i++) { const x = originX + (graphWidth / 10) * i; ctx.beginPath(); ctx.moveTo(x, padding); ctx.lineTo(x, canvasHeight - padding); ctx.stroke() }
    for (let i = 0; i <= 10; i++) { const y = originY - (graphHeight / 10) * i; ctx.beginPath(); ctx.moveTo(padding, y); ctx.lineTo(width - padding, y); ctx.stroke() }
    ctx.strokeStyle = tc.primary; ctx.lineWidth = 2; ctx.beginPath(); ctx.moveTo(originX, originY); ctx.lineTo(originX + graphWidth, originY); ctx.stroke(); const yAxisX = originX + (-xMin) * scaleX; ctx.beginPath(); ctx.moveTo(yAxisX, padding); ctx.lineTo(yAxisX, canvasHeight - padding); ctx.stroke()
    ctx.fillStyle = withAlpha(tc.info, 0.2); ctx.beginPath(); ctx.moveTo(originX + (-zAlpha2 - xMin) * scaleX, originY)
    ...
    ctx.strokeStyle = tc.error; ctx.lineWidth = 2; ctx.setLineDash([8, 4]); ctx.beginPath(); ctx.moveTo(leftX, originY); ctx.lineTo(leftX, padding); ctx.stroke(); ctx.beginPath(); ctx.moveTo(rightX, originY); ctx.lineTo(rightX, padding); ctx.stroke(); ctx.setLineDash([])
    ctx.fillStyle = tc.error; ctx.font = 'bold 13px "Noto Serif SC", serif'; ctx.fillText(`-z_α/2 = ${(-zAlpha2).toFixed(3)}`, leftX - 50, padding + 20); ctx.fillText(`z_α/2 = ${zAlpha2.toFixed(3)}`, rightX - 20, padding + 20)
    const thetaL = mu - zAlpha2 * sigmaXbar; const thetaU = mu + zAlpha2 * sigmaXbar
    ctx.fillStyle = tc.success; ctx.font = 'bold 13px "Noto Serif SC", serif'; ctx.fillText(`置信区间: [${thetaL.toFixed(3)}, ${thetaU.toFixed(3)}]`, originX, padding - 20); ctx.fillText(`置信水平: ${(confidence * 100).toFixed(1)}%`, originX, padding - 5)
    ctx.fillStyle = tc.bgCard; ctx.fillRect(0, canvasHeight, width, infoBarHeight); ctx.strokeStyle = tc.border; ctx.lineWidth = 1; ctx.beginPath(); ctx.moveTo(0, canvasHeight); ctx.lineTo(width, canvasHeight); ctx.stroke()
    ctx.fillStyle = tc.primary; ctx.font = '13px "Noto Serif SC", serif'; ctx.fillText(`1-α = ${(confidence * 100).toFixed(1)}%`, 20, canvasHeight + 30); ctx.fillStyle = tc.error; ctx.fillText(`z_α/2 = ${zAlpha2.toFixed(3)}`, 150, canvasHeight + 30)
    ctx.fillStyle = tc.success; ctx.fillText(`[${thetaL.toFixed(3)}, ${thetaU.toFixed(3)}]`, 310, canvasHeight + 30)
  }, [modelState.params])
  ...
  const drawHypothesisTestingBasics = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    for (let i = 0; i <= 10; i++) { const x = originX + (graphWidth / 10) * i; ctx.beginPath(); ctx.moveTo(x, padding); ctx.lineTo(x, canvasHeight - padding); ctx.stroke() }
    for (let i = 0; i <= 10; i++) { const y = originY - (graphHeight / 10) * i; ctx.beginPath(); ctx.moveTo(padding, y); ctx.lineTo(width - padding, y); ctx.stroke() }
    ctx.strokeStyle = tc.primary; ctx.lineWidth = 2; ctx.beginPath(); ctx.moveTo(originX, originY); ctx.lineTo(originX + graphWidth, originY); ctx.stroke(); const yAxisX = originX + (mu0 - xMin) * scaleX; ctx.beginPath(); ctx.moveTo(yAxisX, padding); ctx.lineTo(yAxisX, canvasHeight - padding); ctx.stroke()
    const acceptLeft = mu0 - zAlpha2; const acceptRight = mu0 + zAlpha2
    ctx.fillStyle = withAlpha(tc.success, 0.2); ctx.beginPath(); ctx.moveTo(originX + (acceptLeft - xMin) * scaleX, originY)
    ...
    ctx.fillStyle = withAlpha(tc.error, 0.3); ctx.beginPath(); ctx.moveTo(originX, originY)
    ...
    ctx.strokeStyle = tc.error; ctx.lineWidth = 2; ctx.setLineDash([8, 4]); ctx.beginPath(); ctx.moveTo(critLeftPx, originY); ctx.lineTo(critLeftPx, padding); ctx.stroke(); ctx.beginPath(); ctx.moveTo(critRightPx, originY); ctx.lineTo(critRightPx, padding); ctx.stroke(); ctx.setLineDash([])
    ctx.fillStyle = tc.error; ctx.font = 'bold 12px "Noto Serif SC", serif'; ctx.fillText(`${(mu0 - zAlpha2).toFixed(2)}`, critLeftPx - 20, originY + 25); ctx.fillText(`${(mu0 + zAlpha2).toFixed(2)}`, critRightPx - 10, originY + 25)
    ctx.fillText('拒绝域', critLeftPx - 50, originY - graphHeight / 2); ctx.fillText('拒绝域', critRightPx + 10, originY - graphHeight / 2)
    ctx.fillStyle = tc.success; ctx.font = 'bold 12px "Noto Serif SC", serif'; ctx.fillText('接受域', (critLeftPx + critRightPx) / 2 - 20, originY - graphHeight / 3)
    ctx.fillStyle = tc.primaryDark; ctx.font = 'bold 13px "Noto Serif SC", serif'; ctx.fillText(`假设检验: H₀: μ = ${mu0} vs H₁: μ ≠ ${mu0}`, originX, padding - 10)
    ctx.fillStyle = tc.bgCard; ctx.fillRect(0, canvasHeight, width, infoBarHeight); ctx.strokeStyle = tc.border; ctx.lineWidth = 1; ctx.beginPath(); ctx.moveTo(0, canvasHeight); ctx.lineTo(width, canvasHeight); ctx.stroke()
    ctx.fillStyle = tc.primary; ctx.font = '13px "Noto Serif SC", serif'; ctx.fillText(`α = ${alpha.toFixed(3)}`, 20, canvasHeight + 30); ctx.fillStyle = tc.error; ctx.fillText(`z_α/2 = ${zAlpha2.toFixed(3)}`, 120, canvasHeight + 30)
    ctx.fillStyle = tc.success; ctx.fillText(`接受域: [${(mu0 - zAlpha2).toFixed(3)}, ${(mu0 + zAlpha2).toFixed(3)}]`, 280, canvasHeight + 30)
  }, [modelState.params])
  ...
  const drawMeanTesting = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    for (let i = 0; i <= 10; i++) { const y = originY - (graphHeight / 10) * i; ctx.beginPath(); ctx.moveTo(padding, y); ctx.lineTo(width - padding, y); ctx.stroke() }
    ctx.strokeStyle = tc.primary; ctx.lineWidth = 2; ctx.beginPath(); ctx.moveTo(originX, originY); ctx.lineTo(originX + graphWidth, originY); ctx.stroke(); const yAxisX = originX + (mu0 - xMin) * scaleX; ctx.beginPath(); ctx.moveTo(yAxisX, padding); ctx.lineTo(yAxisX, canvasHeight - padding); ctx.stroke()
    ctx.fillStyle = withAlpha(tc.error, 0.25); ctx.beginPath(); ctx.moveTo(originX, originY)
    ...
    for (let px = 0; px <= graphWidth; px++) { const x = xMin + px / scaleX; if (x >= mu0 + critXbar) ctx.lineTo(originX + px, originY - pdfXbar(x) * scaleY) }
    ctx.lineTo(originX + graphWidth, originY); ctx.closePath(); ctx.fill()
    ctx.fillStyle = withAlpha(tc.success, 0.15); ctx.beginPath(); ctx.moveTo(originX + (mu0 - critXbar - xMin) * scaleX, originY)
    ...
    ctx.strokeStyle = tc.error; ctx.lineWidth = 2; ctx.setLineDash([8, 4]); ctx.beginPath(); ctx.moveTo(critLPx, originY); ctx.lineTo(critLPx, padding); ctx.stroke(); ctx.beginPath(); ctx.moveTo(critRPx, originY); ctx.lineTo(critRPx, padding); ctx.stroke(); ctx.setLineDash([])
    ctx.fillStyle = tc.error; ctx.font = 'bold 12px "Noto Serif SC", serif'; ctx.fillText(`${(mu0 - critXbar).toFixed(3)}`, critLPx - 30, originY + 25); ctx.fillText(`${(mu0 + critXbar).toFixed(3)}`, critRPx - 10, originY + 25)
    ctx.fillStyle = tc.primaryDark; ctx.font = 'bold 13px "Noto Serif SC", serif'; ctx.fillText(`Z检验: Z = √n(X̄-μ₀)/σ, H₀: μ = ${mu0}`, originX, padding - 15); ctx.fillText(`拒绝域: |Z| > z_α/2 = ${zAlpha2.toFixed(3)}`, originX, padding - 2)
    ctx.fillStyle = tc.bgCard; ctx.fillRect(0, canvasHeight, width, infoBarHeight); ctx.strokeStyle = tc.border; ctx.lineWidth = 1; ctx.beginPath(); ctx.moveTo(0, canvasHeight); ctx.lineTo(width, canvasHeight); ctx.stroke()
    ctx.fillStyle = tc.primary; ctx.font = '13px "Noto Serif SC", serif'; ctx.fillText(`n = ${n}`, 20, canvasHeight + 30); ctx.fillText(`σ = ${sigma.toFixed(1)}`, 100, canvasHeight + 30)
    ctx.fillStyle = tc.error; ctx.fillText(`α = ${alpha.toFixed(3)}`, 200, canvasHeight + 30); ctx.fillText(`z_α/2 = ${zAlpha2.toFixed(3)}`, 320, canvasHeight + 30)
  }, [modelState.params])
  ...
  const drawVarianceTesting = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    ctx.lineTo(originX + graphWidth, originY); ctx.closePath(); ctx.fill()
    ctx.fillStyle = withAlpha(tc.success, 0.15); ctx.beginPath(); ctx.moveTo(originX + chi2Alpha2 * scaleX, originY)
    ...
    ctx.fillStyle = tc.error; ctx.font = 'bold 12px "Noto Serif SC", serif'
    ...
    ctx.fillStyle = tc.primaryDark; ctx.font = 'bold 13px "Noto Serif SC", serif'; ctx.fillText(`χ²检验: (n-1)S²/σ₀² ~ χ²(${df})`, originX, padding - 15); ctx.fillText(`H₀: σ² = ${sigma0Sq.toFixed(1)}`, originX, padding - 2)
    ctx.fillStyle = tc.bgCard; ctx.fillRect(0, canvasHeight, width, infoBarHeight); ctx.strokeStyle = tc.border; ctx.lineWidth = 1; ctx.beginPath(); ctx.moveTo(0, canvasHeight); ctx.lineTo(width, canvasHeight); ctx.stroke()
    ctx.fillStyle = tc.primary; ctx.font = '13px "Noto Serif SC", serif'; ctx.fillText(`n = ${n}, df = ${df}`, 20, canvasHeight + 30); ctx.fillStyle = tc.error; ctx.fillText(`α = ${alpha.toFixed(3)}`, 170, canvasHeight + 30); ctx.fillText(`σ₀² = ${sigma0Sq.toFixed(1)}`, 300, canvasHeight + 30)
  }, [modelState.params])
  ...
  const drawTwoSampleTesting = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    for (let i = 0; i <= 10; i++) { const x = originX + (graphWidth / 10) * i; ctx.beginPath(); ctx.moveTo(x, padding); ctx.lineTo(x, canvasHeight - padding); ctx.stroke() }
    for (let i = 0; i <= 10; i++) { const y = originY - (graphHeight / 10) * i; ctx.beginPath(); ctx.moveTo(padding, y); ctx.lineTo(width - padding, y); ctx.stroke() }
    ctx.strokeStyle = tc.primary; ctx.lineWidth = 2; ctx.beginPath(); ctx.moveTo(originX, originY); ctx.lineTo(originX + graphWidth, originY); ctx.stroke(); const yAxisX = originX + (0 - xMin) * scaleX; ctx.beginPath(); ctx.moveTo(yAxisX, padding); ctx.lineTo(yAxisX, canvasHeight - padding); ctx.stroke()
    ...
    ctx.stroke()
    ctx.strokeStyle = tc.error; ctx.lineWidth = 2; ctx.setLineDash([6, 3]); ctx.beginPath()
    ...
    ctx.stroke(); ctx.setLineDash([])
    const critVal = zAlpha2 * sigUnderH0; ctx.fillStyle = withAlpha(tc.error, 0.2); ctx.beginPath(); ctx.moveTo(originX, originY)
    ...
    ctx.strokeStyle = tc.error; ctx.lineWidth = 2; ctx.setLineDash([8, 4]); ctx.beginPath(); ctx.moveTo(critLPx, originY); ctx.lineTo(critLPx, padding); ctx.stroke(); ctx.beginPath(); ctx.moveTo(critRPx, originY); ctx.lineTo(critRPx, padding); ctx.stroke(); ctx.setLineDash([])
    ctx.fillStyle = tc.error; ctx.font = 'bold 12px "Noto Serif SC", serif'; ctx.fillText('H₀下分布', originX + graphWidth - 100, padding + 15)
    ctx.fillStyle = tc.info; ctx.fillText('真实分布', originX + graphWidth - 100, padding + 30)
    ctx.fillStyle = tc.primaryDark; ctx.font = 'bold 13px "Noto Serif SC", serif'; ctx.fillText(`两样本均值差检验: H₀: μ₁ = μ₂`, originX, padding - 15); ctx.fillText(`Z = (X̄₁-X̄₂)/σ√(1/n₁+1/n₂)`, originX, padding - 2)
    ctx.fillStyle = tc.bgCard; ctx.fillRect(0, canvasHeight, width, infoBarHeight); ctx.strokeStyle = tc.border; ctx.lineWidth = 1; ctx.beginPath(); ctx.moveTo(0, canvasHeight); ctx.lineTo(width, canvasHeight); ctx.stroke()
    ctx.fillStyle = tc.primary; ctx.font = '13px "Noto Serif SC", serif'; ctx.fillText(`n₁ = ${n1}`, 20, canvasHeight + 30); ctx.fillText(`n₂ = ${n2}`, 100, canvasHeight + 30)
    ctx.fillStyle = tc.error; ctx.fillText(`α = ${alpha.toFixed(3)}`, 200, canvasHeight + 30); ctx.fillText(`z_α/2 = ${zAlpha2.toFixed(3)}`, 320, canvasHeight + 30)
  }, [modelState.params])
  ...
  const draw = useCallback(() => {
    ...
    // 应用Canvas缩放与平移变换
    const { scale, offsetX, offsetY } = canvasTransform.current
    ctx.save()
    ...
    } else if (knowledge.id === 'matrix-rank') {
  ...
  // 切换知识点时重置Canvas视图
  useEffect(() => {
    canvasTransform.current = { scale: 1, offsetX: 0, offsetY: 0 }
    setCanvasZoom(1)
  }, [knowledge.id])
  ...
  const handleWheel = (e: React.WheelEvent) => {
    ...
    if (e.ctrlKey || e.metaKey) {
      ...
      const newScale = Math.max(0.3, Math.min(5, canvasTransform.current.scale + delta))
      canvasTransform.current.scale = newScale
      setCanvasZoom(newScale)
      draw()
    } else if (e.shiftKey) {
      // Shift+滚轮：水平平移
      canvasTransform.current.offsetX += e.deltaY > 0 ? -30 : 30
      draw()
      ...
      // 无滑块时：垂直平移
      canvasTransform.current.offsetY += e.deltaY > 0 ? -30 : 30
      draw()
    }
  }
  ...
  const handleMouseDown = (e: React.MouseEvent) => {
    // 中键拖拽 或 右键拖拽 或 Alt+左键拖拽
    if (e.button === 1 || e.button === 2 || (e.button === 0 && e.altKey)) {
      isDragging.current = true
      dragStart.current = {
        x: e.clientX,
        y: e.clientY,
        offsetX: canvasTransform.current.offsetX,
        offsetY: canvasTransform.current.offsetY
      }
      e.preventDefault()
    }
  }
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    canvasTransform.current.offsetX = dragStart.current.offsetX + dx
    canvasTransform.current.offsetY = dragStart.current.offsetY + dy
    draw()
  }
  ...
  // 缩放按钮
  const handleZoomIn = () => {
    const newScale = Math.min(5, canvasTransform.current.scale + 0.2)
    canvasTransform.current.scale = newScale
    setCanvasZoom(newScale)
    draw()
  }
  ...
  const handleZoomOut = () => {
    const newScale = Math.max(0.3, canvasTransform.current.scale - 0.2)
    canvasTransform.current.scale = newScale
    setCanvasZoom(newScale)
    draw()
  }
  ...
  // 触摸事件支持（移动端双指缩放 + 单指拖拽）
  const touchState = useRef({ startDist: 0, startScale: 1, startOffsetX: 0, startOffsetY: 0, startX: 0, startY: 0 })
  ...
  const handleTouchStart = (e: React.TouchEvent) => {
    if (e.touches.length === 2) {
      ...
      touchState.current.startScale = canvasTransform.current.scale
    } else if (e.touches.length === 1) {
      // 单指：准备拖拽
      touchState.current.startX = e.touches[0].clientX
      touchState.current.startY = e.touches[0].clientY
      touchState.current.startOffsetX = canvasTransform.current.offsetX
      touchState.current.startOffsetY = canvasTransform.current.offsetY
    }
  }
  ...
  const handleTouchMove = (e: React.TouchEvent) => {
    ...
    if (e.touches.length === 2) {
      ...
      const newScale = Math.max(0.3, Math.min(5, touchState.current.startScale * ratio))
      canvasTransform.current.scale = newScale
      setCanvasZoom(newScale)
      draw()
    } else if (e.touches.length === 1) {
      ...
      canvasTransform.current.offsetX = touchState.current.startOffsetX + dx
      canvasTransform.current.offsetY = touchState.current.startOffsetY + dy
      draw()
    }
  }
  ...
  return (
    <div className="knowledge-view">
      ...
      {/* 主内容区 - 动态模型 */}
      {hasModel && <div className={`main-visualization ${compareMode ? 'compare-mode' : ''}`}>
        <div className="model-section">
          ...
          <div className="control-toolbar">
            ...
            <div className="toolbar-right">
              <button className="control-btn zoom-btn" onClick={handleZoomOut} title="缩小">
                <ZoomOut size={16} />
              </button>
              <span className="zoom-indicator" onClick={handleResetView} title="点击重置视图" style={{ cursor: 'pointer' }}>
                {Math.round(canvasZoom * 100)}%
              </span>
              <button className="control-btn zoom-btn" onClick={handleZoomIn} title="放大">
                <ZoomIn size={16} />
              </button>
              {canvasZoom !== 1 && (
                <button className="control-btn zoom-btn" onClick={handleResetView} title="重置视图">
                  <RotateCcw size={14} />
                </button>
              )}
          ...
          >
            ...
            {canvasTransform.current.scale !== 1 && (
              <div className="canvas-hint">Alt+拖拽平移 · Ctrl+滚轮缩放 · 点击百分比重置</div>
            )}
          </div>
